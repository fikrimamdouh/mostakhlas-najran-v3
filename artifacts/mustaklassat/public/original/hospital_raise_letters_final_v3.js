...
function hasCompleteMap(o){
  var keys=['index','final','labor','noPrev','salary','vacancies','vacations','saudi','custom'];
  if(!o||typeof o!=='object')return false;
  return keys.every(function(k){return !!o[k]&&typeof o[k]==='object';});
}
function sanitizeSettings(){
  try{
    ...
    if(!hasCompleteMap(s.letters))delete s.letters;
    if(!hasCompleteMap(s.layout))delete s.layout;
...
function loadEngine(){
  ...
  s.src='/original/hospital_raise_letters_engine_v8.js?v=20260630_hospital_admin_model_loader_v5_complete_maps';
  s.async=false;
  s.onload=function(){console.info('[HospitalLetters] engine loaded v8 hospital admin model loader v5 complete maps');};
  s.onerror=showFail;
