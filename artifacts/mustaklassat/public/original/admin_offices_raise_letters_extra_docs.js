...
(function () {
  ...
  const PERF_DEDUCTION_KEY = 'adminOfficePerformanceDeductions_v1';
  ...
  function firstValue() { for (const v of arguments) { const s = clean(v); if (s && s !== 'غير محدد' && s !== 'undefined' && s !== 'null' && s !== '—') return s; } return ''; }
  function num(v) {
    const ar = '٠١٢٣٤٥٦٧٨٩';
    const fa = '۰۱۲۳۴۵۶۷۸۹';
  ...
  function getData() {
    ...
    return candidates.reduce((best, item) => count(item) > count(best) ? item : best, {});
  }

  function getCompanyName() {
    const c = getContract();
    return firstValue(c.contractorName, c.contractorCompany, c.companyName, c.company, c.contractor, c.supplierName, c.vendorName, localStorage.getItem('companyName'), document.querySelector('.companyName')?.textContent) || 'غير محدد';
  }
  ...
  function paymentNo() {
    const e = getExtract();
    const raw = firstValue(e.paymentNumber, e.extractNumber, e.paymentNo, e.extractNo, localStorage.getItem('paymentNumber'), localStorage.getItem('extractNumber'), localStorage.getItem('paymentNo')) || '—';
  ...
  function calcEmployee(emp) {
    ...
    if (typeof window.calculateAdminOfficeEmployeeFinancials === 'function') {
      return window.calculateAdminOfficeEmployeeFinancials(emp, { totalDaysInMonth: p.totalDaysInMonth, daysInExtract: p.daysInExtract, contractType: c.contractType || 'عقد أساسي', directPurchaseRatio: num(c.directPurchaseRatio) });
    }
  ...
  function calcSite(key) {
    ...
    return (getData()[key] || []).reduce((out, emp) => {
      const c = calcEmployee(emp);
      out.cost += num(c.costForPeriod); out.absenceDeduction += num(c.deduction); out.absenceFine += num(c.absenceFine); out.nationalityFine += num(c.nationalityFine); out.totalDeduction += num(c.deduction) + num(c.absenceFine) + num(c.nationalityFine); out.finalNet += num(c.netSalary);
      return out;
  ...
  function groupTotals() {
    ...
    orderedKeys().forEach(key => { const target = isWorkshopKey(key) ? out.workshops : out.offices; const c = calcSite(key); ['cost', 'absenceDeduction', 'absenceFine', 'nationalityFine', 'totalDeduction', 'finalNet'].forEach(k => { target[k] += num(c[k]); }); });
    return out;
  }
  ...
  function generateEntitlementStatement() {
    const rows = orderedKeys().map(key => { const c = calcSite(key); const name = getNames()[key] || key; return `<tr><td>${esc(name)}</td><td>${esc(monthLabel())}</td><td>${moneySAR(c.cost)}</td><td>${moneySAR(c.absenceDeduction)}</td><td>${moneySAR(c.absenceFine)}</td><td>${moneySAR(c.nationalityFine)}</td><td>${moneySAR(c.totalDeduction)}</td><td>${moneySAR(c.finalNet)}</td></tr>`; }).join('');
  ...
  function performanceDeduction(centerKey) {
    ...
    const newMap = readJson(PERF_DEDUCTION_KEY, {});
    const oldMap = readJson('performanceDeductions', {});
    return num(newMap[centerKey] !== undefined ? newMap[centerKey] : oldMap[centerKey]);
  }

  function monthlyPerformancePage(centerKey) {
    const site = getNames()[centerKey] || centerKey;
    const amount = performanceDeduction(centerKey);
    const p = getPeriod();
    return `<section class="page">${headerHtml()}<div class="cert-title">شهادة الأداء الشهري لأعمال النظافة والصيانة والتشغيل غير الطبي</div><div class="sub-title">لموقع : ${esc(site)}<br>عن دفعة رقم (${esc(paymentNo())}) من ${esc(formatDate(p.startDate))} م إلى ${esc(formatDate(p.endDate))} م</div><table class="amount-table"><thead><tr><th>القسم</th><th>إجمالي مبلغ الحسميات</th></tr></thead><tbody><tr><td>أداء الصيانة</td><td>${moneySAR(amount)}</td></tr></tbody></table>${signatureHtml(true)}${footerHtml()}</section>`;
  }

  function generateMonthlyPerformanceCertificates() {
    const keys = orderedKeys();
    if (!keys.length) return alert('لا توجد مواقع متاحة لإصدار شهادة الأداء الشهري.');
    openPrintDoc('شهادة الأداء الشهري لكل موقع', keys.map(monthlyPerformancePage).join(''));
  }

  function dateForIndex(index) { const p = getPeriod(); const d = new Date(p.startDate); d.setDate(p.startDate.getDate() + index); return formatDate(d); }
  function buildSegments(type) {
    const names = getNames(), data = getData(), matcher = type === 'absence' ? isAbsenceCode : isVacationCode, out = [];
    orderedKeys().forEach(centerKey => (data[centerKey] || []).forEach(emp => {
      const calc = calcEmployee(emp), days = calc.days || [];
      let start = -1;
      for (let i = 0; i <= days.length; i++) {
        const matched = i < days.length && matcher(days[i]);
        if (matched && start < 0) start = i;
        if ((!matched || i === days.length) && start >= 0) {
          out.push({ key: `${type}|${centerKey}|${emp.iqamaId || emp.name || ''}|${start}|${i - 1}`, site: names[centerKey] || centerKey, jobTitle: emp.jobTitle || '', from: dateForIndex(start), to: dateForIndex(i - 1), note: '' });
          start = -1;
        }
      }
    }));
  ...
  function installButtons() {
    ...
    section.innerHTML = `<h3>خطابات وبيانات إضافية</h3><div class="btn-row"><button class="btn btn-gold" onclick="AdminOfficesExtraDocs.generateGroupedMonthlyEntitlementCertificate()">شهادة الاستحقاق الشهري المجمعة</button><button class="btn btn-gold" onclick="AdminOfficesExtraDocs.generateEntitlementStatement()">بيان استحقاق</button><button class="btn btn-gold" onclick="AdminOfficesExtraDocs.generateSalaryDeliveryCertificate()">شهادة تسليم الرواتب</button><button class="btn btn-gold" onclick="AdminOfficesExtraDocs.generateMonthlyPerformanceCertificates()">شهادة الأداء الشهري</button><button class="btn btn-gold" onclick="AdminOfficesExtraDocs.openEditableStatement('absence')">بيان الغيابات</button><button class="btn btn-gold" onclick="AdminOfficesExtraDocs.openEditableStatement('vacation')">بيان الإجازات</button></div>`;
  ...
  window.AdminOfficesExtraDocs = { generateGroupedMonthlyEntitlementCertificate, generateEntitlementStatement, generateSalaryDeliveryCertificate, generateMonthlyPerformanceCertificates, openEditableStatement, printEditableStatement, saveNote };
  observe();
  installButtons();
  setTimeout(installButtons, 700);
  setTimeout(installButtons, 1800);
  setTimeout(installButtons, 3200);
  console.info('[Admin Offices Extra Docs] installed inside raise letters page v3 performance certificate');
})();