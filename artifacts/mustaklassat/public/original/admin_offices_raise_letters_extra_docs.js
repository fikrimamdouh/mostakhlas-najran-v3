...
function pageFinalRaiseLetter() {
  ...
  function generateFullExtractPrint() { const ks = centerKeys(); if (!ks.length) return void window.NajranDialogs.alert('لا توجد مواقع/مكاتب لطباعة المستخلص الكامل.'); let html = pageFinalRaiseLetter() + pageNoPreviousPayment() + pageRaiseSummary() + pageGroupedCertificate() + pageEntitlementStatement() + pageSalaryCertificate() + pageSaudiEmployees() + pageStatement('absence') + pageStatement('vacation'); ks.forEach(k => { html += pageSiteRaise(k) + pageAttendance(k) + pagePerformance(k) + pagePerformanceCertificate(k) + pageAchievement(k); }); openPrintDoc('طباعة المستخلص كامل', html); }
  ...
  function saveFinalSettingsFromSection() { const patch = {}; document.querySelectorAll('#final-raise-letter-settings [data-rl-setting]').forEach(el => { patch[el.dataset.rlSetting] = el.value; }); saveSettingsPatch(patch); if (window.AdminOfficesRaiseLetters && typeof window.AdminOfficesRaiseLetters.saveDialog === 'function') { try { window.AdminOfficesRaiseLetters.saveDialog(); } catch (_) {} } void window.NajranDialogs.alert('تم حفظ إعدادات خطاب الرفع النهائي.'); }
  ...
  window.AdminOfficesExtraDocs = {
    ...
    generateMonthlyPerformanceCertificates: () => { const ks = centerKeys(); if (!ks.length) return void window.NajranDialogs.alert('لا توجد مواقع متاحة لإصدار شهادة الأداء الشهري.'); openPrintDoc('شهادة الأداء الشهري لكل موقع', ks.map(pagePerformanceCertificate).join('')); },
