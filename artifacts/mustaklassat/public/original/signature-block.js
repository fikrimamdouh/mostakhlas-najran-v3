...
(function (global) {
  ...
  const CSS = [
    ...
    '.sb-dialog-footer{display:flex;gap:10px;justify-content:center}.sb-save-btn,.sb-dialog-footer #sb-save-btn{padding:10px 30px!important;border:none!important;border-radius:12px!important;background:#16a34a!important;color:#fff!important;font-family:inherit!important;font-size:14px!important;font-weight:800!important;cursor:pointer!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;min-width:140px!important}.sb-cancel-btn{padding:10px 30px;border:1.5px solid #e2e8f0;border-radius:12px;background:#fff;color:#64748b;font-family:inherit;font-size:14px;font-weight:700;cursor:pointer}',
...
panel.querySelector('.sbcp-reset').onclick=()=>{consumablesStyleDraft=defaultDetailedStyle();consumablesStyleDraft.savedAt=new Date().toISOString();writeDetailedStyle(consumablesStyleDraft);syncConsumablesStyleToCloud();renderAll('consumables');buildConsumablesPrintStylePanel(target);};panel.querySelector('.sbcp-final').onclick=()=>finalConsumablesPrint(target);function bindRange(sel,prop,suffix){const input=panel.querySelector(sel);if(!input)return;input.oninput=()=>{consumablesStyleDraft[prop]=Number(input.value);setRangeText(input,consumablesStyleDraft[prop],suffix);apply();};}bindRange('.sbcp-title-font','titleFontSize','px');bindRange('.sbcp-name-font','nameFontSize','px');bindRange('.sbcp-title-weight','titleWeight','');bindRange('.sbcp-name-weight','nameWeight','');bindRange('.sbcp-title-gap','titleNameGap','px');bindRange('.sbcp-name-line','nameLineHeight','');bindRange('.sbcp-letter','letterSpacing','px');bindRange('.sbcp-block-top','blockTop','px');bindRange('.sbcp-block-y','blockY','px');bindRange('.sbcp-card-w','cardWidth','px');bindRange('.sbcp-card-h','cardHeight','px');bindRange('.sbcp-border-radius','cardBorderRadius','px');bindRange('.sbcp-card-padding','cardPadding','px');bindRange('.sbcp-gap','gap','px');bindRange('.sbcp-line-length','lineLength','px');bindRange('.sbcp-line-gap','lineNameGap','px');bindRange('.sbcp-stamp-size','stampSize','px');bindRange('.sbcp-stamp-gap','stampGap','px');panel.querySelector('.sbcp-text-only').onchange=e=>{consumablesStyleDraft.textOnly=e.target.checked;apply();};panel.querySelector('.sbcp-border-visible').onchange=e=>{consumablesStyleDraft.cardBorderVisible=e.target.checked;apply();};panel.querySelector('.sbcp-border-width').onchange=e=>{consumablesStyleDraft.cardBorderWidth=Number(e.target.value);apply();};panel.querySelector('.sbcp-border-color').onchange=e=>{consumablesStyleDraft.cardBorderColor=e.target.value;apply();};panel.querySelector('.sbcp-card-bg').onchange=e=>{consumablesStyleDraft.cardBackground=e.target.value;apply();};panel.querySelector('.sbcp-show-line').onchange=e=>{consumablesStyleDraft.showLine=e.target.checked;apply();};panel.querySelector('.sbcp-show-stamp').onchange=e=>{consumablesStyleDraft.showStamp=e.target.checked;apply();};panel.querySelector('.sbcp-per-row').onchange=e=>{consumablesStyleDraft.perRow=e.target.value;apply();};panel.querySelector('.sbcp-text-align').onchange=e=>{consumablesStyleDraft.textAlign=e.target.value;apply();};panel.querySelector('.sbcp-stamp-pos').onchange=e=>{consumablesStyleDraft.stampPosition=e.target.value;apply();};panel.querySelectorAll('[data-sig-idx]').forEach(group=>{const idx=Number(group.getAttribute('data-sig-idx'));while(consumablesStyleDraft.perSignature.length<=idx)consumablesStyleDraft.perSignature.push(normalizePerSig({}));const sig=consumablesStyleDraft.perSignature[idx];const x=group.querySelector('.sbcp-sig-x'),y=group.querySelector('.sbcp-sig-y'),a=group.querySelector('.sbcp-sig-align');x.oninput=()=>{sig.x=Number(x.value);setRangeText(x,sig.x,'px');apply();};y.oninput=()=>{sig.y=Number(y.value);setRangeText(y,sig.y,'px');apply();};a.onchange=()=>{sig.textAlign=a.value||null;apply();};});}
  ...
  // المراقب يُركَّب فقط على الصفحة التي تخدمها الدالة فعلًا.
  // قبل ذلك كان مركّبًا على body في كل صفحة تُحمِّل signature-block،
  // فيبني المتصفح سجل تغيير لكل إدراج عنصر (آلاف عند رسم الجداول)
  // لاستدعاء دالة تخرج فورًا لأن الصفحة ليست admin_offices_attendance.
  if(/admin_offices_attendance\.html/.test(location.href)){
