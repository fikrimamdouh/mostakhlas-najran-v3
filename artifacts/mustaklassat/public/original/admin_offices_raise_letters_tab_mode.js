(function(){
  ...
  function patchMainButton(){
    ...
    if(btn&&!btn.__raiseLettersTabPatched){btn.onclick=function(e){if(e){e.preventDefault();e.stopPropagation();}openStandaloneTab();};btn.__raiseLettersTabPatched=true;btn.title='فتح خطابات الرفع في تبويب مستقل';}
  }
  ...
  function cleanDuplicateStandaloneShell(){
    ...
    const overlays=[...document.querySelectorAll('#raise-letters-overlay')];
  ...
  function openStandalone(){
    ...
    if(api&&typeof api.openDialog==='function'){
      opened=true;
      try{api.openDialog();setTimeout(cleanDuplicateStandaloneShell,80);setTimeout(cleanDuplicateStandaloneShell,400);}catch(_){opened=false;}
    }
  }
  function boot(n){
    loadLettersHelpers();
  ...
  document.addEventListener('click',function(){setTimeout(function(){loadLettersHelpers();patchMainButton();if(isStandalone)cleanDuplicateStandaloneShell();},120);},true);
  console.info('[Admin Offices Raise Letters] standalone tab mode installed v6 single overlay only');
})();