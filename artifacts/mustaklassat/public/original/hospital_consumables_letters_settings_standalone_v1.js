// Standalone Consumables Letters Settings V2
(function () {
  'use strict';
  ...
  function defaults() {
    return {
      version: 'standalone-consumables-settings-v2',
      selectedDoc: 'main',
  ...
  function save(s) { s.version = 'standalone-consumables-settings-v2'; localStorage.setItem(KEY, JSON.stringify(s)); }
  ...
  function note(t) {
    ...
    if (t) setTimeout(function () { if (n && n.textContent === t) n.textContent = ''; }, 2200);
  }

  function signatures(s, k) {
    var arr = s.letters[k].signatures || [];
    return '<section class="section"><h2>تواقيع هذا الخطاب فقط</h2><div class="grid">' + arr.map(function (x, i) {
  ...
  function letterFields(s, k) {
    ...
    h += '</div></section>';
    return h + signatures(s, k);
  }

  function render() {
    var s = read(), k = s.selectedDoc || 'main';
    document.body.innerHTML = '<main class="box"><h1>إعدادات خطابات رفع المستهلكات</h1><p>صفحة مستقلة تحفظ إعدادات المستهلكات فقط، ولا تفتح خطابات الرفع العادية.</p><div id="msg"></div><div class="actions top"><button id="save" class="ok">حفظ</button><button id="open">فتح صفحة المستهلكات</button><button id="back" class="danger">رجوع</button></div><div class="actions docs">' + DOCS.map(function (d) { return '<button type="button" data-doc="' + d[0] + '" class="doc ' + (k === d[0] ? 'active' : '') + '">' + d[1] + '</button>'; }).join('') + '</div><section class="section"><h2>الترويسة المشتركة</h2><div class="grid">' + yn('letterheadEnabled', 'تفعيل الترويسة', s.letterheadEnabled || 'no') + yn('letterheadHasPlaceData', 'الترويسة تحتوي بيانات الجهة والمكان', s.letterheadHasPlaceData || 'yes') + field('contentTop', 'بداية المحتوى بعد الترويسة mm', s.contentTop || 52, 'number') + field('vatRate', 'نسبة الضريبة', s.vatRate || 15, 'number') + '<label class="field wide"><span>رفع صورة الترويسة A4</span><input id="headFile" type="file" accept="image/*">' + (s.letterheadDataUrl ? '<img class="preview" src="' + esc(s.letterheadDataUrl) + '">' : '') + '</label></div><div class="actions"><button type="button" id="deleteHead">حذف الترويسة</button></div></section>' + letterFields(s, k) + '</main>';
    bind();
  }

  function bind() {
    document.querySelectorAll('[data-p]').forEach(function (el) { el.oninput = readForm; el.onchange = readForm; });
    ...
    document.getElementById('deleteHead').onclick = function () { var s = readForm(); s.letterheadDataUrl = ''; s.letterheadEnabled = 'no'; save(s); render(); };
    ...
    document.getElementById('headFile').onchange = function () { var f = this.files && this.files[0]; if (!f) return; var r = new FileReader(); r.onload = function () { var s = readForm(); s.letterheadDataUrl = String(r.result || ''); s.letterheadEnabled = 'yes'; save(s); render(); }; r.readAsDataURL(f); };
  }
