...
// Clean center + per-document settings + optional A4 letterhead + print offset.
// ===================================================================
(function () {
  'use strict';
  ...
  function writeJson(key, value) {
    try {
      ...
      alert('تعذر الحفظ. غالباً حجم صورة الترويسة كبير. صغّر الصورة ثم ارفعها مرة أخرى.');
      return false;
    }
  }
  ...
  function firstValue() {
    for (let i = 0; i < arguments.length; i++) {
      ...
      if (v && !['غير محدد', 'undefined', 'null', '—', '-'].includes(v)) return v;
    }
    return '';
  }
  ...
  function normalizeDigits(v) {
    const ar = '٠١٢٣٤٥٦٧٨٩';
    const fa = '۰۱۲۳۴۵۶۷۸۹';
    return clean(v).replace(/[٠-٩]/g, d => ar.indexOf(d)).replace(/[۰-۹]/g, d => fa.indexOf(d));
  }

  function normalizePaymentNo(v) {
  ...
  function settings() {
    ...
    out.documentSettings = Object.assign({}, stored.documentSettings || {});
    return out;
  }

  function saveAll(next) {
    return writeJson(SETTINGS_KEY, Object.assign(defaults(), next || {}));
  }
  ...
  function docSettings(key) {
    const s = settings();
    return Object.assign({ enabled: false, useGlobalLetterhead: true }, (s.documentSettings || {})[key] || {});
  }

  function effective(key) {
    const s = settings();
    const d = docSettings(key);
    if (!d.enabled) return s;
    const out = Object.assign({}, s);
    Object.keys(d).forEach(k => {
      if (['enabled', 'useGlobalLetterhead'].includes(k)) return;
      if (d[k] !== '' && d[k] != null) out[k] = d[k];
    });
    if (d.useGlobalLetterhead !== false) {
      out.letterheadEnabled = s.letterheadEnabled;
      out.letterheadMode = s.letterheadMode;
      out.letterheadDataUrl = s.letterheadDataUrl;
      out.letterheadHeightMm = s.letterheadHeightMm;
      out.contentTopMm = s.contentTopMm;
      out.blankLines = s.blankLines;
      out.lineHeightMm = s.lineHeightMm;
      out.printScale = s.printScale;
    }
    return out;
  }
  ...
  function meta() {
    ...
    const s = settings();
    return {
      start: toIsoDate(firstValue(e.extractStart, e.periodStart, e.startDate, e.fromDate, localStorage.getItem('extractStart'), localStorage.getItem('periodStart'), s.extractStart, s.period2Start, s.period1Start)),
      end: toIsoDate(firstValue(e.extractEnd, e.periodEnd, e.endDate, e.toDate, localStorage.getItem('extractEnd'), localStorage.getItem('periodEnd'), s.extractEnd, s.period2End, s.period1End)),
      paymentNo: normalizePaymentNo(firstValue(e.paymentNumber, e.extractNumber, e.paymentNo, localStorage.getItem('paymentNumber'), localStorage.getItem('extractNumber'), s.paymentNo, s.paymentNumber, s.extractNumber))
    };
  }
  ...
  function getAttendance() {
    ...
    arr.push(readJson('adminOfficesAttendanceData_v1', {}), readJson('adminOfficesAttendanceData', {}));
    const count = o => Object.values(o || {}).reduce((s, r) => s + (Array.isArray(r) ? r.length : 0), 0);
    return arr.reduce((b, o) => count(o) > count(b) ? o : b, {});
  }

  function officeKeys() {
    const n = getNames();
    const d = getAttendance();
  ...
  function input(name, label, type, scope, val) {
    return `<label class="rl-field"><span>${esc(label)}</span><input type="${esc(type || 'text')}" ${scope === 'doc' ? 'data-doc-field' : 'data-global-field'}="${esc(name)}" value="${esc(val ?? '')}"></label>`;
  }

  function area(name, label, scope, val) {
    return `<label class="rl-field wide"><span>${esc(label)}</span><textarea ${scope === 'doc' ? 'data-doc-field' : 'data-global-field'}="${esc(name)}">${esc(val ?? '')}</textarea></label>`;
  }

  function select(name, label, scope, val, opts) {
    return `<label class="rl-field"><span>${esc(label)}</span><select ${scope === 'doc' ? 'data-doc-field' : 'data-global-field'}="${esc(name)}">${opts.map(o => `<option value="${esc(o[0])}" ${String(val) === String(o[0]) ? 'selected' : ''}>${esc(o[1])}</option>`).join('')}</select></label>`;
  }

  function button(action, text, cls, key) {
    return `<button type="button" class="rl-btn ${esc(cls || '')}" data-action="${esc(action)}" ${key ? `data-doc-key="${esc(key)}"` : ''}>${esc(text)}</button>`;
  }

  function saveGlobalFromUI() {
    const s = settings();
    document.querySelectorAll('#admin-raise-clean [data-global-field]').forEach(el => {
      s[el.dataset.globalField] = el.type === 'checkbox' ? el.checked : el.value;
    });
    saveAll(s);
    UI.globalOpen = true;
  ...
  function saveDocFromUI() {
    const s = settings();
    const key = document.getElementById('rl-doc-select')?.value || s.selectedDocKey || 'finalRaise';
    s.selectedDocKey = key;
    s.documentSettings = s.documentSettings || {};
    const d = Object.assign({}, s.documentSettings[key] || {});
    document.querySelectorAll('#admin-raise-clean [data-doc-field]').forEach(el => {
      d[el.dataset.docField] = el.type === 'checkbox' ? el.checked : el.value;
    });
    d.enabled = !!document.getElementById('rl-doc-enabled')?.checked;
    d.useGlobalLetterhead = !!document.getElementById('rl-doc-use-global-letterhead')?.checked;
    s.documentSettings[key] = d;
    saveAll(s);
    UI.docOpen = true;
    flash('تم حفظ إعدادات خطاب: ' + docLabel(key));
    render();
  }
  ...
  function copyGlobalToDoc() {
    const s = settings();
    const key = document.getElementById('rl-doc-select')?.value || s.selectedDocKey || 'finalRaise';
    const d = { enabled: true, useGlobalLetterhead: true };
    TEXT_FIELDS.concat(AREA_FIELDS).concat(NUM_FIELDS).forEach(x => { d[x[0]] = s[x[0]] || ''; });
    d.contentTopMm = s.contentTopMm || 0;
    d.blankLines = s.blankLines || 0;
    d.lineHeightMm = s.lineHeightMm || LINE_MM;
    d.printScale = s.printScale || 100;
    s.documentSettings = s.documentSettings || {};
    s.documentSettings[key] = d;
    s.selectedDocKey = key;
    saveAll(s);
    UI.docOpen = true;
    flash('تم نسخ العام إلى خطاب: ' + docLabel(key));
    render();
  }
  ...
  function clearDoc() {
    ...
    const key = document.getElementById('rl-doc-select')?.value || s.selectedDocKey || 'finalRaise';
    s.documentSettings = s.documentSettings || {};
    s.documentSettings[key] = { enabled: false, useGlobalLetterhead: true };
    s.selectedDocKey = key;
    saveAll(s);
    UI.docOpen = true;
    flash('تم تفريغ إعدادات خطاب: ' + docLabel(key));
    render();
  }

  function copyDocToDoc() {
    const from = document.getElementById('rl-doc-select')?.value || settings().selectedDocKey || 'finalRaise';
    const to = document.getElementById('rl-copy-to-doc')?.value;
    if (!to || to === from) return alert('اختر خطاباً مختلفاً للنسخ إليه.');
    const s = settings();
    s.documentSettings = s.documentSettings || {};
    s.documentSettings[to] = Object.assign({}, s.documentSettings[from] || {}, { enabled: true });
    s.selectedDocKey = to;
    saveAll(s);
    UI.docOpen = true;
    flash('تم نسخ إعدادات ' + docLabel(from) + ' إلى ' + docLabel(to));
    render();
  }

  function resetAllSettings() {
    if (!confirm('سيتم حذف كل إعدادات الخطابات والترويسة. تأكيد؟')) return;
    localStorage.removeItem(SETTINGS_KEY);
    UI.globalOpen = true;
    UI.docOpen = false;
    render();
    flash('تمت إعادة ضبط إعدادات الخطابات.');
  }
  ...
  function exportSettings() {
    ...
    a.download = 'admin_offices_raise_letters_settings.json';
  ...
  function importSettings(file) {
    ...
    r.onload = function () {
      try {
        ...
        const next = Object.assign(defaults(), imported || {});
        next.documentSettings = imported.documentSettings || {};
        saveAll(next);
        UI.globalOpen = true;
        UI.docOpen = false;
        render();
        flash('تم استيراد إعدادات الخطابات.');
      } catch (err) {
  ...
  function uploadLetterhead(file) {
    if (!file) return;
    if (!/^image\//.test(file.type || '')) return alert('ارفع صورة ترويسة فقط.');
    if (file.size > 1000000 && !confirm('حجم الصورة أكبر من 1MB وقد يبطئ الحفظ. الأفضل ضغطها. الاستمرار؟')) return;
    const r = new FileReader();
    r.onload = function () {
      const s = settings();
      ...
      saveAll(s);
      UI.globalOpen = true;
      flash('تم رفع الترويسة A4.');
      render();
    };
    r.readAsDataURL(file);
  }

  function totalTopMm(eff) {
    return Number(eff.contentTopMm || 0) + Number(eff.blankLines || 0) * Number(eff.lineHeightMm || LINE_MM);
  }

  function printCss(eff) {
    const top = totalTopMm(eff);
    const scale = Math.max(70, Math.min(130, Number(eff.printScale || 100))) / 100;
    const img = eff.letterheadEnabled && eff.letterheadDataUrl ? String(eff.letterheadDataUrl) : '';
    const mode = eff.letterheadMode || 'external';
    const h = Number(eff.letterheadHeightMm || 45);
    let before = '';
    ...
    return `<style id="rl-clean-print-letterhead">@media screen,print{body{padding-top:${top}mm!important;transform:scale(${scale})!important;transform-origin:top center!important}${before}}@media print{.toolbar,.print-toolbar,.no-print{display:none!important}}</style>`;
  }

  function decoratePrintHtml(html, eff) {
    if (typeof html !== 'string') return html;
    const css = printCss(eff);
    if (/rl-clean-print-letterhead/.test(html)) return html;
    if (/<\/head>/i.test(html)) return html.replace(/<\/head>/i, css + '</head>');
    return css + html;
  }

  function installPrintGuard() {
    if (window.__RL_CLEAN_PRINT_GUARD__) return;
    window.__RL_CLEAN_PRINT_GUARD__ = true;
    const nativeOpen = window.open;
    window.open = function () {
      const w = nativeOpen.apply(window, arguments);
      patchPrintWindow(w);
      setTimeout(() => patchPrintWindow(w), 50);
      setTimeout(() => patchPrintWindow(w), 150);
      return w;
    };
  }

  function patchPrintWindow(w) {
    try {
      if (!w || !w.document || w.document.__RL_CLEAN_DOC_PATCHED__) return;
      w.document.__RL_CLEAN_DOC_PATCHED__ = true;
      const oldWrite = w.document.write.bind(w.document);
      w.document.write = function () {
        const eff = window.__RL_CLEAN_ACTIVE_PRINT_SETTINGS__ || settings();
        const args = Array.from(arguments).map(x => decoratePrintHtml(x, eff));
        return oldWrite.apply(w.document, args);
      };
    } catch (_) {}
  }

  function applySiteSelectionForLegacy(key) {
    const single = document.getElementById('rl-site-select');
    if (key === 'oneSite' && single && !single.value) {
      const first = officeKeys()[0];
      if (first) single.value = first;
    }
  }

  function withDocSettings(key, fn) {
    installPrintGuard();
    applySiteSelectionForLegacy(key);
    const raw = localStorage.getItem(SETTINGS_KEY);
    const base = settings();
    const eff = effective(key);
    window.__RL_CLEAN_ACTIVE_PRINT_SETTINGS__ = eff;
    const merged = Object.assign({}, base, eff, { documentSettings: base.documentSettings, selectedDocKey: key });
    ...
    try {
      ...
      console.error('[RaiseLettersClean]', key, err);
      alert('تعذر تنفيذ ' + docLabel(key) + '\n' + (err.message || err));
    } finally {
      setTimeout(() => {
  ...
  function renderGlobal() {
    const s = settings();
    return `<section id="rl-settings" class="settings-panel ${UI.globalOpen ? 'open' : ''}">
      <h3>الإعدادات العامة لكل الخطابات</h3>
      <div class="rl-note">هذه القيم يستخدمها كل خطاب ما لم تفعل إعداداً مستقلاً لهذا الخطاب.</div>
      <div class="rl-grid">
        ${TEXT_FIELDS.map(f => input(f[0], f[1], f[2] || 'text', 'global', s[f[0]])).join('')}
        ${NUM_FIELDS.map(f => input(f[0], f[1], 'number', 'global', s[f[0]])).join('')}
        ${AREA_FIELDS.map(f => area(f[0], f[1], 'global', s[f[0]])).join('')}
        ${select('letterheadMode', 'طريقة الترويسة', 'global', s.letterheadMode, [['external', 'مطبوع خارجي — لا تطبع صورة وتُنزل المحتوى فقط'], ['full', 'صورة A4 كاملة كخلفية'], ['top', 'صورة ترويسة علوية فقط']])}
        ${input('contentTopMm', 'نزول بداية الخطاب من أعلى الورقة — ملم', 'number', 'global', s.contentTopMm)}
        ${input('blankLines', 'نزول إضافي بعدد السطور', 'number', 'global', s.blankLines)}
        ${input('lineHeightMm', 'ارتفاع السطر بالملليمتر', 'number', 'global', s.lineHeightMm || LINE_MM)}
        ${input('letterheadHeightMm', 'ارتفاع الترويسة العلوية — ملم', 'number', 'global', s.letterheadHeightMm)}
        ${input('printScale', 'تكبير/تصغير الطباعة %', 'number', 'global', s.printScale)}
      </div>
      <div class="rl-upload">
        <label><input type="checkbox" data-global-field="letterheadEnabled" ${s.letterheadEnabled ? 'checked' : ''}> تفعيل الترويسة</label>
        <input type="file" id="rl-letterhead-file" accept="image/*">
        <button type="button" class="rl-btn" data-action="clearLetterhead">حذف الترويسة</button>
        ${s.letterheadDataUrl ? '<span class="ok">ترويسة محفوظة</span>' : '<span class="warn">لا توجد ترويسة مرفوعة</span>'}
      </div>
      <div class="rl-row">
        ${button('saveGlobal', 'حفظ الإعدادات العامة', 'primary')}
        ${button('exportSettings', 'تصدير الإعدادات', '')}
        <label class="rl-btn file-btn">استيراد الإعدادات<input type="file" id="rl-import-file" accept="application/json,.json" hidden></label>
        ${button('resetAll', 'إعادة ضبط المصنع', 'danger')}
      </div>
    </section>`;
  }
  ...
  function renderDocSettings() {
    const s = settings();
    const key = s.selectedDocKey || 'finalRaise';
    const d = docSettings(key);
    return `<section id="rl-doc-settings" class="settings-panel doc-panel ${UI.docOpen ? 'open' : ''}">
      <h3>إعداد مستقل لكل خطاب</h3>
      <div class="rl-note">اختر الخطاب، فعّل الإعداد المستقل، ثم عدّل المخاطب والنص والتوقيع ونزول الورقة لهذا الخطاب وحده.</div>
      <div class="rl-doc-top">
        <label class="rl-field"><span>اختر الخطاب</span><select id="rl-doc-select">${DOCS.map(x => `<option value="${esc(x.key)}" ${x.key === key ? 'selected' : ''}>${esc(x.label)}</option>`).join('')}</select></label>
        <label class="switch"><input type="checkbox" id="rl-doc-enabled" ${d.enabled ? 'checked' : ''}> تفعيل إعداد مستقل لهذا الخطاب</label>
        <label class="switch"><input type="checkbox" id="rl-doc-use-global-letterhead" ${d.useGlobalLetterhead !== false ? 'checked' : ''}> استخدام ترويسة الإعداد العام</label>
      </div>
      <div class="rl-grid">
        ${TEXT_FIELDS.map(f => input(f[0], f[1], f[2] || 'text', 'doc', d[f[0]] || '')).join('')}
        ${NUM_FIELDS.map(f => input(f[0], f[1], 'number', 'doc', d[f[0]] || '')).join('')}
        ${AREA_FIELDS.map(f => area(f[0], f[1], 'doc', d[f[0]] || '')).join('')}
        ${select('letterheadMode', 'طريقة ترويسة هذا الخطاب', 'doc', d.letterheadMode || 'external', [['external', 'مطبوع خارجي — نزول فقط'], ['full', 'صورة A4 كاملة'], ['top', 'صورة علوية فقط']])}
        ${input('contentTopMm', 'نزول هذا الخطاب من أعلى الورقة — ملم', 'number', 'doc', d.contentTopMm || '')}
        ${input('blankLines', 'نزول إضافي لهذا الخطاب بعدد السطور', 'number', 'doc', d.blankLines || '')}
        ${input('lineHeightMm', 'ارتفاع السطر لهذا الخطاب — ملم', 'number', 'doc', d.lineHeightMm || '')}
        ${input('letterheadHeightMm', 'ارتفاع الترويسة العلوية لهذا الخطاب — ملم', 'number', 'doc', d.letterheadHeightMm || '')}
        ${input('printScale', 'تكبير/تصغير هذا الخطاب %', 'number', 'doc', d.printScale || '')}
      </div>
      <div class="rl-row">
        ${button('saveDoc', 'حفظ إعداد هذا الخطاب', 'primary')}
        ${button('copyGlobalToDoc', 'نسخ العام لهذا الخطاب', '')}
        ${button('clearDoc', 'تفريغ هذا الخطاب', 'danger')}
        ${button(key, 'تجربة طباعة هذا الخطاب', 'site')}
      </div>
      <div class="rl-row copy-row">
        <label class="rl-field copy-select"><span>نسخ إعداد هذا الخطاب إلى</span><select id="rl-copy-to-doc">${DOCS.map(x => `<option value="${esc(x.key)}" ${x.key === key ? 'disabled' : ''}>${esc(x.label)}</option>`).join('')}</select></label>
        ${button('copyDocToDoc', 'نسخ خطاب إلى خطاب آخر', '')}
      </div>
    </section>`;
  }
  ...
  function renderSites() {
    ...
    return `<div class="rl-site-top"><label class="rl-field"><span>الموقع المحدد لخطاب واحد</span><select id="rl-site-select"><option value="">اختر الموقع</option>${keys.map(k => `<option value="${esc(k)}">${esc(names[k] || k)}</option>`).join('')}</select></label><div class="rl-phrase">${extractPhrase()}</div></div><div class="rl-sites">${keys.map(k => `<label class="rl-site"><input type="checkbox" class="rl-site-check" value="${esc(k)}" checked><span>${esc(names[k] || k)}</span></label>`).join('') || '<b>لا توجد مواقع محفوظة.</b>'}</div>`;
  }

  function docButtons(group, cls) {
    return DOCS.filter(d => d.group === group).map(d => `<div class="rl-doc-card"><div><b>${esc(d.label)}</b><small>${docSettings(d.key).enabled ? 'إعداد مستقل مفعل' : 'يستخدم الإعداد العام'}</small></div><div>${button(d.key, 'طباعة', cls, d.key)}${button('openDocSettings', 'إعدادات', '', d.key)}</div></div>`).join('');
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    const b = e.target.closest && e.target.closest('#admin-raise-clean [data-action]');
    if (!root || !b) return;
    e.preventDefault();
    const a = b.dataset.action;
    const k = b.dataset.docKey;
    if (k) {
      const s = settings();
      s.selectedDocKey = k;
      saveAll(s);
    }
    if (a === 'toggleGlobal') { UI.globalOpen = !UI.globalOpen; return render(); }
    if (a === 'toggleDoc') { UI.docOpen = !UI.docOpen; return render(); }
    if (a === 'openDocSettings') { UI.docOpen = true; return render(); }
    if (a === 'saveGlobal') return saveGlobalFromUI();
    if (a === 'saveDoc') return saveDocFromUI();
    if (a === 'copyGlobalToDoc') return copyGlobalToDoc();
    if (a === 'clearDoc') return clearDoc();
    if (a === 'copyDocToDoc') return copyDocToDoc();
    if (a === 'exportSettings') return exportSettings();
    if (a === 'resetAll') return resetAllSettings();
    if (a === 'clearLetterhead') {
      const s = settings();
      s.letterheadDataUrl = '';
      s.letterheadEnabled = false;
      saveAll(s);
      UI.globalOpen = true;
      flash('تم حذف الترويسة.');
      return render();
    }
  ...
  document.addEventListener('change', function (e) {
    if (e.target && e.target.id === 'rl-letterhead-file') return uploadLetterhead(e.target.files && e.target.files[0]);
    if (e.target && e.target.id === 'rl-import-file') return importSettings(e.target.files && e.target.files[0]);
    if (e.target && e.target.id === 'rl-doc-select') {
      const s = settings();
      s.selectedDocKey = e.target.value;
      saveAll(s);
      UI.docOpen = true;
      return render();
    }
  ...
  console.info('[Admin Offices Raise Letters Clean V1] installed complete controls');
})();