(function () {
  ...
  async function authToken() {
    ...
    for (var i = 0; i < getters.length; i++) {
      if (typeof getters[i] === 'function') {
        try { var fresh = await getters[i]({ skipCache: true }); if (fresh) return fresh; } catch (_) {}
      }
    }
    try {
  ...
  function buildPermitNode(payload, offscreen) {
    ...
    var rows = [
      ...
      ['وقت بداية الزيارة', formatDate(v.startsAt, true)],
      ['وقت نهاية الزيارة', formatDate(v.endsAt, true)],
      ...
      ['الغرض من الزيارة', esc(v.purpose || '—')]
    ];
    ...
    node.innerHTML = draft +
      ...
      '<div style="display:flex;justify-content:space-between;align-items:flex-end;border:1.5px solid #d1d5db;border-radius:10px;padding:13px 20px">' +
        '<div><p style="font-weight:800;font-size:14px;color:#1e3c72;margin:0 0 6px">مدير إدارة الصيانة</p>' +
        '<p style="font-size:13px;margin:0 0 4px">الاسم: ' + esc(settings.managerName || 'م. محمد عباس المكرمي') + '</p>' +
        '<p style="font-size:13px;margin:0 0 8px">التاريخ: ' + formatDate(v.approvedAt, false) + ' م</p>' + signatureHtml + '</div>' +
        '<div style="display:flex;align-items:flex-end;gap:14px"><div style="text-align:center">' +
          '<img src="' + esc(v.qrDataUrl || '') + '" alt="QR تصريح الزيارة" style="width:105px;height:105px;display:block;image-rendering:pixelated">' +
          '<div style="font-size:10px;font-weight:800;margin-top:3px;color:#1e3c72">' + esc(v.serialNumber || 'مسودة') + '</div></div>' + stampHtml + '</div></div>';
    return node;
  }
