...
(function () {
  ...
  const DEPT_NAMES = {
    ...
    security: 'الأمن', laundry: 'المغسلة', patient_services: 'خدمات المرضى',
    'patient-services': 'خدمات المرضى', admin_saudi: 'الإداريون السعوديون', 'admin-saudi': 'الإداريون السعوديون'
  };

  const STATUS_LABELS = {
    'ح': 'حاضر', 'غ': 'غائب', 'ج': 'إجازة', 'ش': 'شاغرة', 'ت': 'تحت الإجراء', 'ب': 'بداية العقد', 'ن': 'نهاية العقد'
  };

  const SECTION_DEFS = [
    {
      id: 'summary', title: 'الملخص وبيانات العقد', icon: '📌',
      keys: ['persistentContractData', 'persistentExtractData', 'contractData', 'contractDetails', 'hospitalName', 'companyName', 'contractNumber', 'extractMonth', 'extractYear', 'extractStart', 'extractEnd', 'finalLaborCost', 'finalConsumablesCost', 'grand-net-total']
    },
    {
      id: 'attendance', title: 'الحضور والانصراف', icon: '📋',
    ...
    {
      id: 'performance', title: 'تقييم الأداء', icon: '📊',
      ...
      prefixes: ['dept_', 'performance_', 'ng_', 'nd_']
    },
    {
      id: 'achievement', title: 'الإنجاز', icon: '🏆',
    ...
    {
      id: 'consumables', title: 'المستهلكات والمواد', icon: '🧪',
    ...
    {
      id: 'spare', title: 'قطع الغيار', icon: '🔩',
      keys: ['spare_partsData', 'sparePartsTotalAmount'], prefixes: ['spare_']
    },
    {
      id: 'signatures', title: 'التوقيعات والاعتماد الداخلي', icon: '✍️',
  ...
  function getGlobalExtractById(id) {
    try {
      if (typeof allExtracts !== 'undefined' && Array.isArray(allExtracts)) return allExtracts.find(e => String(e.id) === String(id)) || null;
    } catch (_) {}
    return null;
  }
  ...
  function getSectionEntries(snapshot, def) {
    ...
    Object.keys(snapshot || {}).forEach(rawKey => {
      ...
      const exactHit = exact.has(nk);
      const prefixHit = prefixes.some(p => nk.startsWith(p));
      if (exactHit || prefixHit) out.push({ key: nk, originalKey: rawKey, value: snapshot[rawKey] });
    });
    return out;
  }
  ...
  function renderScalar(v) {
    ...
    if (typeof v === 'number') return esc(Number(v).toLocaleString('en-US', { maximumFractionDigits: 2 }));
  ...
  function tableFromArray(arr) {
    ...
    const preferred = ['name','employeeName','jobTitle','category','nationality','iqamaId','attendanceDays','absenceDays','deduction','absencePenalty','totalFine','netSalary','salary','total','amount','item','quantity','price'];
    ...
    return '<div class="nr-table-wrap"><table class="nr-table"><thead><tr>' + cols.map(c => '<th>' + esc(labelFor(c)) + '</th>').join('') + '</tr></thead><tbody>' +
      rows.map(r => '<tr>' + cols.map(c => '<td>' + renderScalar(r ? r[c] : '') + '</td>').join('') + '</tr>').join('') +
  ...
  function labelFor(k) {
    return {
      ...
      attendanceDays: 'حضور', absenceDays: 'غياب', deduction: 'الحسم', absencePenalty: 'غرامة الغياب', totalFine: 'إجمالي الغرامة', netSalary: 'صافي الاستحقاق', salary: 'الراتب',
      total: 'الإجمالي', amount: 'المبلغ', item: 'الصنف', quantity: 'الكمية', price: 'السعر'
    }[k] || k;
  }

  function countDays(days) {
    const counts = { present:0, absent:0, leave:0, vacant:0, pending:0, start:0, end:0, other:0 };
    (Array.isArray(days) ? days : []).forEach(d => {
      if (d === 'ح') counts.present++;
      else if (d === 'غ') counts.absent++;
      else if (d === 'ج') counts.leave++;
      else if (d === 'ش') counts.vacant++;
      else if (d === 'ت') counts.pending++;
      else if (d === 'ب') counts.start++;
      else if (d === 'ن') counts.end++;
      else counts.other++;
    });
    return counts;
  }

  function renderDaysStrip(days) {
    if (!Array.isArray(days) || days.length === 0) return '<span class="nr-muted">—</span>';
    return '<div class="nr-days-strip">' + days.map((d, i) => '<span class="nr-day nr-day-' + esc(d) + '" title="يوم ' + (i + 1) + ' — ' + esc(STATUS_LABELS[d] || d) + '">' + esc(d || '—') + '</span>').join('') + '</div>';
  }
  ...
  function renderAttendanceData(data) {
    ...
    const deptCards = depts.map(deptKey => {
      ...
      let dPresent = 0, dAbsent = 0, dLeave = 0, dVacant = 0, dPending = 0, dNet = 0, dDed = 0, dFine = 0;
      rows.forEach(emp => {
        const c = countDays(emp.days);
        dPresent += Number(emp.attendanceDays ?? c.present ?? 0);
        dAbsent += Number(emp.absenceDays ?? c.absent ?? 0);
        dLeave += c.leave; dVacant += c.vacant; dPending += c.pending;
        dNet += Number(emp.netSalary || 0);
        dDed += Number(emp.deduction || 0);
        dFine += Number(emp.totalFine || emp.absencePenalty || 0);
      });
      totalEmployees += rows.length; totalPresent += dPresent; totalAbsent += dAbsent; totalNet += dNet; totalDeduction += dDed; totalFine += dFine;

      const body = rows.map((emp, idx) => {
        const c = countDays(emp.days);
        return '<tr>' +
          '<td>' + (idx + 1) + '</td>' +
          '<td class="nr-emp-name">' + esc(emp.name || emp.employeeName || '—') + '</td>' +
          '<td>' + esc(emp.jobTitle || '—') + '</td>' +
          '<td>' + esc(emp.category || '—') + '</td>' +
          '<td>' + esc(emp.nationality || '—') + '</td>' +
          '<td>' + esc(emp.iqamaId || '—') + '</td>' +
          '<td>' + (emp.attendanceDays ?? c.present) + '</td>' +
          '<td>' + (emp.absenceDays ?? c.absent) + '</td>' +
          '<td>' + c.leave + '</td>' +
          '<td>' + c.vacant + '</td>' +
          '<td>' + c.pending + '</td>' +
          '<td>' + renderScalar(emp.deduction) + '</td>' +
          '<td>' + renderScalar(emp.totalFine || emp.absencePenalty) + '</td>' +
          '<td>' + renderScalar(emp.netSalary) + '</td>' +
          '<td>' + renderDaysStrip(emp.days) + '</td>' +
        '</tr>';
      }).join('');

      return '<div class="nr-att-card"><div class="nr-att-head"><strong>' + esc(DEPT_NAMES[deptKey] || deptKey) + '</strong><span>' + rows.length + ' موظف</span><span>غياب: ' + dAbsent + '</span><span>الصافي: ' + dNet.toLocaleString('en-US', { maximumFractionDigits: 2 }) + '</span></div>' +
        '<div class="nr-table-wrap"><table class="nr-table nr-att-table"><thead><tr><th>#</th><th>الموظف</th><th>الوظيفة</th><th>الفئة</th><th>الجنسية</th><th>الإقامة</th><th>حضور</th><th>غياب</th><th>إجازة</th><th>شاغرة</th><th>تحت الإجراء</th><th>الحسم</th><th>الغرامة</th><th>الصافي</th><th>أيام الشهر</th></tr></thead><tbody>' + body + '</tbody></table></div></div>';
    }).join('');

    const summary = '<div class="nr-summary-grid">' +
      '<div><b>' + totalEmployees + '</b><span>إجمالي الموظفين</span></div>' +
      '<div><b>' + totalPresent + '</b><span>أيام حضور</span></div>' +
      '<div><b>' + totalAbsent + '</b><span>أيام غياب</span></div>' +
      '<div><b>' + totalDeduction.toLocaleString('en-US', { maximumFractionDigits: 2 }) + '</b><span>إجمالي الحسم</span></div>' +
      '<div><b>' + totalFine.toLocaleString('en-US', { maximumFractionDigits: 2 }) + '</b><span>إجمالي الغرامات</span></div>' +
      '<div><b>' + totalNet.toLocaleString('en-US', { maximumFractionDigits: 2 }) + '</b><span>صافي الاستحقاق</span></div>' +
    '</div>';

    return summary + deptCards;
  }
  ...
  function renderValue(value) {
    ...
    if (Array.isArray(value)) {
      ...
      return '<div class="nr-list">' + value.slice(0, 120).map(x => '<div class="nr-list-item">' + renderScalar(x) + '</div>').join('') + '</div>';
    }
    if (value && typeof value === 'object') {
      ...
      if (firstArrayKey) return '<div class="nr-subtitle">' + esc(firstArrayKey) + '</div>' + tableFromArray(value[firstArrayKey]);
  ...
  function renderSection(snapshot, def) {
    ...
    return '<div class="nr-section" id="nr-sec-' + esc(def.id) + '"><h3>' + def.icon + ' ' + esc(def.title) + '</h3>' +
      entries.map(entry => {
        const isAttendance = def.id === 'attendance' && /attendanceData/i.test(entry.key);
        const body = isAttendance ? renderAttendanceData(entry.value) : renderValue(entry.value);
        return '<div class="nr-block"><div class="nr-key">' + esc(entry.key) + (entry.originalKey !== entry.key ? ' <span>(' + esc(entry.originalKey) + ')</span>' : '') + '</div>' + body + '</div>';
      }).join('') + '</div>';
  }
  ...
  window.openNajranReviewDetails = function (id) {
    ...
    document.getElementById('nr-tabs').innerHTML = SECTION_DEFS.map((def, idx) => '<button class="nr-tab ' + (idx === 0 ? 'active' : '') + '" data-id="' + esc(def.id) + '" onclick="window.activateNajranReviewSection(\'' + esc(def.id) + '\')">' + def.icon + ' ' + esc(def.title) + '</button>').join('');
