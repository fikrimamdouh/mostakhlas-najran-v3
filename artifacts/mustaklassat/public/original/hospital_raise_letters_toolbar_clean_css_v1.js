// Hospital Raise Letters Toolbar Clean CSS V3
...
// Does not rename or visually alter the index button.
(function () {
  'use strict';

  if (!/hospital_raise_letters\.html/.test(location.pathname) && !window.__HOSPITAL_LETTERS_STANDALONE_PAGE__) return;
  if (window.__HOSPITAL_RAISE_LETTERS_TOOLBAR_CLEAN_CSS_V3__) return;
  window.__HOSPITAL_RAISE_LETTERS_TOOLBAR_CLEAN_CSS_V3__ = true;

  function ensureCss() {
    if (document.getElementById('hrl-toolbar-clean-css-v3')) return;
    var st = document.createElement('style');
    st.id = 'hrl-toolbar-clean-css-v3';
    st.textContent = [
  ...
  console.info('[Hospital Raise Letters Toolbar Clean CSS] installed v3 no index visual change');
})();