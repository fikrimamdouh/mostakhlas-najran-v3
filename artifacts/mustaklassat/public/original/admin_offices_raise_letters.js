...
(function () {
  ...
  function saveSettings(patch) {
    ...
    [
      ...
      'period1LaborNet',
      'period2LaborNet',
      'consumablesPeriod1Net',
      'consumablesPeriod2Net',
      'consumablesNet',
      'purchasePeriodLabel'
  ...
  function toHtml(settings) {
    return `<div class="to"><span class="recipient-name">${esc(settings.recipient)}</span><span class="recipient-suffix">${esc(settings.recipientSuffix)}</span></div>`;
  }

  function signatureHtml(settings, mode) {
    if (mode === 'two') {
      return `<div class="signatures">
        <div><div class="sig-title">${esc(settings.unitManagerTitle)}</div><div class="line"></div><div class="sig-name">${esc(settings.unitManagerName)}</div></div>
        <div><div class="sig-title">${esc(settings.managerTitle)}</div><div class="line"></div><div class="sig-name">${esc(settings.managerName)}</div></div>
      </div>`;
    }
  ...
  function docCss() {
    return `<style>
      ...
      *{box-sizing:border-box}
      ...
      .to{font-size:15.5px;font-weight:900;margin:4px 0 24px;line-height:2}.recipient-name,.recipient-suffix{display:inline}.recipient-suffix{margin-right:8px;padding-right:0}
  ...
  function laborLetterPage() {
    ...
    return `<section class="page">${headerHtml(s)}${toHtml(s)}<div class="salam">السلام عليكم ورحمة الله وبركاته، وبعد:</div>
      <div class="body-text">نرفق لسعادتكم المستخلص الشهري لشركة ${esc(company)} والخاص ببند العمالة لمواقع ${esc(s.scopeName)}، ${extractPhrase()}.</div>
      ...
      <div class="closing">لذا نأمل بعد الاطلاع إحالته إلى جهة الاختصاص لتدقيقه واستكمال إجراءات صرف مستحقات المقاول / ${esc(company)}.<br>وتقبلوا تحياتنا ،،،</div>
  ...
  function siteLetterPage(centerKey) {
    ...
    return `<section class="page">${headerHtml(s)}${toHtml(s)}<div class="salam">السلام عليكم ورحمة الله وبركاته، وبعد:</div>
      <div class="body-text">إشارة إلى عقد الصيانة والنظافة والتشغيل غير الطبي لمواقع ${esc(s.scopeName)}، مقاولة ${esc(company)}، نرفق لسعادتكم المستخلص الشهري الخاص بموقع ${esc(site)} ببند العمالة، ${extractPhrase()}، بمبلغ وقدره (${moneyPlain(amount)} ريال) ${esc(tafqeetSAR(amount))}.</div>
      <div class="closing">نأمل بعد الاطلاع إحالته إلى جهة الاختصاص لتدقيقه واستكمال إجراءات صرف مستحقات المقاول / ${esc(company)} حسب ما جاء أعلاه.<br>وتقبلوا خالص تحياتنا ،،،</div>
      ${signatureHtml(s, 'one')}${footerHtml(s)}</section>`;
  }
  ...
  function generateDeclaration() {
    ...
    const body = `<section class="page">${headerHtml(s)}<div style="text-align:left;font-size:14px;font-weight:900">التاريخ : ${esc(fmtDate(s.declarationDate))} م</div>
      <div class="report-title" style="text-align:center;font-size:19px;font-weight:900;margin:28px 0 18px">إقرار بعدم أسبقية الصرف</div>
      <div class="body-text">تقر إدارة الصيانة بالشئون الهندسية بصحة نجران بأن مستخلص العمالة لشركة ${esc(company)} لمواقع ${esc(s.scopeName)}، ${extractPhrase()}، بمبلغ وقدره (${money(amount)} ريال) ${esc(tafqeetSAR(amount))}.<br>لم يسبق صرفه من قبلنا.</div>
  ...
  function renderDialog() {
    ...
    const html = `<div class="settings-overlay" id="raise-letters-overlay"><div class="settings-dialog">
      <h2>خطابات رفع عمالة المكاتب الإدارية</h2>
      <div class="btn-row">
        <button class="btn btn-primary" onclick="AdminOfficesRaiseLetters.saveDialog()">حفظ الإعدادات</button>
        <button class="btn btn-gold" onclick="AdminOfficesRaiseLetters.generateLaborRaiseLetter()">خطاب العمالة الإجمالي</button>
        <button class="btn btn-gold" onclick="AdminOfficesRaiseLetters.generateDeclaration()">إقرار عدم أسبقية الصرف</button>
        <button class="btn btn-gold" onclick="AdminOfficesRaiseLetters.generateSiteRaiseLetter()">خطاب الموقع المحدد</button>
        <button class="btn btn-gold" onclick="AdminOfficesRaiseLetters.generateSelectedSiteLetters()">خطابات المواقع المختارة</button>
      ...
      <div class="section-box"><h3>بيانات المستخلص الحالية — قراءة فقط</h3><div class="settings-grid">
        ...
        <div class="field"><label>صافي العمالة الإجمالي</label><div class="readonly-box">${money(total.net)}</div></div>
      </div></div>

      <div class="section-box"><h3>اختيار المواقع لخطابات المواقع</h3><div class="settings-grid">
        <div class="field"><label>الموقع المحدد لخطاب واحد</label><select id="rl-site-select" data-rl-setting="siteLetterDefaultCenter"><option value="">اختر الموقع</option>${options}</select></div>
        <div class="field"><label>تحديد سريع</label><div class="readonly-box"><button type="button" class="btn btn-light" onclick="document.querySelectorAll('.rl-site-check').forEach(cb=>cb.checked=true)">تحديد الكل</button> <button type="button" class="btn btn-light" onclick="document.querySelectorAll('.rl-site-check').forEach(cb=>cb.checked=false)">إلغاء الكل</button></div></div>
      </div><div class="site-checks">${checks}</div></div>

      <div class="section-box"><h3>إعدادات الخطابات المحفوظة</h3><div class="settings-grid">
        ...
        ${settingsField('scopeName', 'نطاق الخطاب الإجمالي')}
      ...
      <div class="section-box"><h3>التواقيع المحفوظة</h3><div class="settings-grid">
  ...
  function openDialog() {
    if (!document.getElementById('raise-letters-overlay')) renderDialog();
  }

  function closeDialog() {
    document.getElementById('raise-letters-overlay')?.remove();
  }
  ...
  console.info('[Admin Offices Raise Letters] installed clean labor/site v3');
})();