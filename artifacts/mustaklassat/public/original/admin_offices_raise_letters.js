...
(function () {
  ...
  function fmtDate(v) {
    ...
    try {
      const d = new Date(v);
      return Number.isNaN(d.getTime()) ? cleanText(v) || 'غير محدد' : d.toLocaleDateString('en-CA');
  ...
  function getManualCompanyName(hospitalName) {
    try { return localStorage.getItem(manualCompanyKey(hospitalName || '')) || ''; } catch (_) { return ''; }
  }

  function normalizeContractSettings() {
    const c = getContract();
    const s = getSession();
    const hospitalName = firstValue(c.hospitalName, localStorage.getItem('hospitalName'), s.hospital, s.reviewActiveHospital);
    const manualCompany = getManualCompanyName(hospitalName);
    const domCompany = document.querySelector('.companyName')?.textContent;
    ...
    if (company) {
      c.contractorName = company;
      c.companyName = company;
      writeJson('persistentContractData', c);
      try { localStorage.setItem('companyName', company); } catch (_) {}
    }
    return { contract: c, companyName: company };
  }

  function getCompanyName() {
    return normalizeContractSettings().companyName || 'غير محدد';
  }
  ...
  function getAttendance() {
    ...
    candidates.push(readJson('admin_offices_attendance_data', {}));

    let best = {};
    candidates.forEach(item => {
      if (countRows(item) > countRows(best)) best = item;
    });

    if (countRows(best) > 0) {
      try { localStorage.setItem('adminOfficesAttendanceData_v1', JSON.stringify(best)); } catch (_) {}
    }
    return best;
  }
  ...
  function getActiveExtractMeta() {
    ...
    const domStart = firstValue(document.getElementById('extract-start-date')?.textContent, document.querySelector('[data-extract-start]')?.textContent, document.querySelector('.extract-start')?.textContent);
    const domEnd = firstValue(document.getElementById('extract-end-date')?.textContent, document.querySelector('[data-extract-end]')?.textContent, document.querySelector('.extract-end')?.textContent);
    const domPayment = firstValue(document.getElementById('payment-number')?.textContent, document.querySelector('[data-payment-no]')?.textContent, document.querySelector('.paymentNo')?.textContent, document.querySelector('.extract-number')?.textContent);

    const paymentNoRaw = firstValue(
      ...
      localStorage.getItem('paymentNumber'), localStorage.getItem('extractNumber'), localStorage.getItem('paymentNo'), localStorage.getItem('extractNo'), localStorage.getItem('batchNumber'), domPayment
    ) || '—';
    ...
    const start = toIsoDate(firstValue(
      ...
      localStorage.getItem('extractStart'), localStorage.getItem('periodStart'), localStorage.getItem('startDate'), domStart
    ));
    ...
    const end = toIsoDate(firstValue(
      ...
      localStorage.getItem('extractEnd'), localStorage.getItem('periodEnd'), localStorage.getItem('endDate'), domEnd
    ));

    const paymentNo = String(paymentNoRaw).match(/^\d+$/) ? String(paymentNoRaw).padStart(2, '0') : String(paymentNoRaw);
    if (start || end || paymentNo !== '—') {
      const updated = Object.assign({}, e);
      if (start) updated.extractStart = start;
      if (end) updated.extractEnd = end;
      if (paymentNo !== '—') updated.paymentNumber = paymentNo;
      writeJson('persistentExtractData', updated);
      if (start) localStorage.setItem('extractStart', start);
      if (end) localStorage.setItem('extractEnd', end);
      if (paymentNo !== '—') {
  ...
  function getPeriod() {
    ...
    const start = new Date(meta.start || Date.now());
    const end = new Date(meta.end || meta.start || Date.now());
  ...
  function calcSite(centerKey) {
    ...
    rows.forEach(emp => {
      if (typeof window.calculateAdminOfficeEmployeeFinancials === 'function') {
        const calc = window.calculateAdminOfficeEmployeeFinancials(emp, { totalDaysInMonth: p.totalDaysInMonth, daysInExtract: p.daysInExtract, contractType: c.contractType || 'عقد أساسي', directPurchaseRatio: ratio });
        out.cost += num(calc.costForPeriod); out.absenceDeduction += num(calc.deduction); out.absenceFine += num(calc.absenceFine); out.nationalityFine += num(calc.nationalityFine); out.fines += num(calc.totalFine); out.netBeforePerformance += num(calc.netSalary); return;
      }
      ...
      let absenceDays = 0;
      days.forEach(d => { const st = window.STATUS_CODES && window.STATUS_CODES[d]; if (st && st.isAbsence) absenceDays++; else if (!st && d === 'غ') absenceDays++; });
      ...
      out.cost += cost; out.absenceDeduction += absenceDeduction; out.absenceFine += absenceFine; out.nationalityFine += natFine; out.fines += absenceFine + natFine; out.netBeforePerformance += net;
    });
  ...
  function readDisplayedGrandNet() {
    const selectors = ['#grand-net-total', '#admin-grand-net-total', '#admin-net-total'];
    for (const sel of selectors) {
      const n = num(document.querySelector(sel)?.textContent || '');
      if (n > 0) return n;
    }
    return num(localStorage.getItem('grand-net-total-admin')) || num(localStorage.getItem('adminOfficesGrandNetTotal')) || num(localStorage.getItem('adminGrandNetTotal')) || 0;
  }
  ...
  function underThousand(n) {
    ...
    if (r) { if (r < 10) parts.push(ones[r]); else if (r < 20) parts.push(teens[r - 10]); else { const o = r % 10; const t = Math.floor(r / 10); parts.push(o ? `${ones[o]} و${tens[t]}` : tens[t]); } }
    return parts.join(' و');
  }

  function scaleName(value, singular, dual, plural) { if (value === 1) return singular; if (value === 2) return dual; if (value >= 3 && value <= 10) return plural; return singular; }
  function intToArabicWords(n) {
    n = Math.floor(num(n)); if (n === 0) return 'صفر';
    const scales = [{ value: 1000000000, singular: 'مليار', dual: 'ملياران', plural: 'مليارات' }, { value: 1000000, singular: 'مليون', dual: 'مليونان', plural: 'ملايين' }, { value: 1000, singular: 'ألف', dual: 'ألفان', plural: 'آلاف' }];
    const parts = [];
    for (const s of scales) { const v = Math.floor(n / s.value); if (v) { if (v === 1) parts.push(s.singular); else if (v === 2) parts.push(s.dual); else parts.push(`${underThousand(v)} ${scaleName(v, s.singular, s.dual, s.plural)}`); n %= s.value; } }
  ...
  function installButton() { const bar = document.getElementById('main-action-buttons') || document.querySelector('.std-action-bar') || document.querySelector('.main-action-buttons'); if (!bar || document.getElementById('raise-letters-btn')) return; const btn = document.createElement('button'); btn.type = 'button'; btn.id = 'raise-letters-btn'; btn.className = 'ab ab-update no-print'; btn.innerHTML = '<i class="fas fa-envelope-open-text"></i> خطابات الرفع'; btn.onclick = openDialog; bar.appendChild(btn); }

  window.AdminOfficesRaiseLetters = { openDialog, closeDialog, saveDialog, generateLaborRaiseLetter, generateDeclaration, generateSiteRaiseLetter, generateSelectedSiteLetters, tafqeetSAR, calcSite, calcGrandLabor, laborValues, syncContractSettings: normalizeContractSettings };
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', installButton); else installButton(); setTimeout(installButton, 700); setTimeout(installButton, 1800); setTimeout(installButton, 3500); console.info('[Admin Offices Raise Letters] installed clean labor/site v5 contract-settings');
})();