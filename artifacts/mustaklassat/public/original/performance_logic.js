...
function printPerformanceCertificate(centerKey) {
    if (typeof buildAdminOfficePerformancePrintHtml !== 'function') {
        void window.NajranDialogs.alert('خطأ: دالة طباعة جدول ح غ 1 غير محملة.');
        return;
    }
    ...
    if (!win) {
        void window.NajranDialogs.alert('المتصفح منع فتح نافذة الطباعة.');
        return;
    }
    ...
    doc.write(`
        ...
        <head>
            <meta charset="UTF-8">
            <title>جدول ح غ 1 - تقييم ${centerName}</title>
        ...
        <body>
            ${buildAdminOfficePerformancePrintHtml(centerKey)}
        </body>
        </html>
    `);
    doc.close();
...
window.buildAdminOfficePerformancePrintHtml = function(centerKey, options = {}) {
    ...
    const centerNames =
        typeof getCenterNames === 'function'
            ? getCenterNames()
    ...
    const items =
        typeof getPerformanceItems === 'function'
    ...
    const rows = items.map((item, index) => {
        ...
        const score =
            typeof getPerformanceScore === 'function'
                ? Number(getPerformanceScore(centerKey, item, index)) || 0
                : Number(item.score ?? item.max) || 0;
    ...
    return `
        <div class="page-container portrait-page-perf exact-hg1-print">
            <div class="hg1-top-lines">
                <div>—</div>
                <div>—</div>
            </div>

            <div class="hg1-contract-box">
                <div>
                    <b>تفاصيل العقد:</b>
                    ${esc(contractData.contractDetails || '—')}
                    |
                    <b>نوع العقد:</b>
                    ${esc(contractData.contractType || 'عقد أساسي')}
                </div>
                <div>
                    <b>مدة المستخلص:</b>
                    من ${fmtDate(extractData.extractStart)}
                    إلى ${fmtDate(extractData.extractEnd)}
                </div>
            </div>

            <div class="hg1-title">
                جدول ح غ 1 - تقييم قسم ${esc(centerName)}
            </div>

            <div class="hg1-section-amount">
                إجمالي المبلغ للقسم: ${money(sectionAmount)} ريال
            </div>

            <table class="hg1-table">
                <thead>
                    <tr>
                        <th>أنشطة القسم</th>
                        <th>الدرجة القصوى</th>
