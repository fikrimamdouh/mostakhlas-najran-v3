...
const PERFORMANCE_TEMPLATE_VERSION = 'hg1_admin_workshop_grouped_2026_06_22_v3';
const PERFORMANCE_SCORE_VERSION = 'hg1_scores_grouped_2026_06_22_v3';

function isWorkshopCenter(centerKey) {
    return centerKey === 'admin_staff';
}

function getDefaultPerformanceItems(centerKey) {
    return JSON.parse(JSON.stringify(isWorkshopCenter(centerKey) ? WORKSHOP_PERFORMANCE_ITEMS : ADMIN_OFFICE_PERFORMANCE_ITEMS));
}

function getDefaultScore(item) {
    const max = parseInt(item.max, 10) || 0;
    const score = item.score !== undefined ? parseInt(item.score, 10) : max;
    if (isNaN(score)) return max;
    return Math.max(0, Math.min(score, max));
}

// --- 2. DATA MANAGEMENT ---

function getPerformanceStorageKey(centerKey) {
    return `performanceItems_hg1_${centerKey}_v3`;
}
...
function savePerformanceItems(centerKey, items) {
    ...
    localStorage.setItem(`${storageKey}_version`, PERFORMANCE_TEMPLATE_VERSION);
}
...
function clearOldPerformanceScoresOnce(centerKey) {
    ...
    if (allPerformanceData[centerKey]) {
        delete allPerformanceData[centerKey];
...
function scoreInputHTML(centerKey, item, index) {
    const score = getPerformanceScore(centerKey, item, index);
    return `<input type="number" inputmode="numeric" lang="en" dir="ltr" class="score-input" style="font-family:Arial,Tahoma,sans-serif;direction:ltr;unicode-bidi:plaintext;text-align:center;width:70px;" data-item-index="${index}" min="0" max="${item.max}" value="${score}" oninput="savePerformanceScoreLive('${centerKey}', this)">`;
}
...
function buildAdminPerformanceRows(centerKey) {
    ...
    items.forEach((item, index) => {
        rows += `<tr><td>${index + 1}</td><td class="item-text">${item.text}</td><td>${item.max}</td><td>${scoreInputHTML(centerKey, item, index)}</td></tr>`;
...
function buildWorkshopPerformanceRows(centerKey, printMode = false) {
    ...
    items.forEach((item, index) => {
        ...
        let activityCells = '';
        if (item.groupedContinuation) {
            activityCells = `<td class="workshop-sub">${item.sub || item.text}</td>`;
        } else if (item.section) {
            activityCells = `<td class="workshop-group" rowspan="${item.rowspan || 1}">${item.section}</td><td class="workshop-sub">${item.sub || ''}</td>`;
        } else {
            activityCells = `<td class="item-text" colspan="2">${item.sub || item.text}</td>`;
        }
...
function buildPerformanceTableHTML(centerKey) {
    if (isWorkshopCenter(centerKey)) {
        const built = buildWorkshopPerformanceRows(centerKey, false);
    ...
    const built = buildAdminPerformanceRows(centerKey);
    return `<table class="performance-table"><thead><tr><th>م</th><th>أنشطة القسم</th><th>الدرجة القصوى</th><th>التقدير</th></tr></thead><tbody>${built.rows}<tr class="total-row"><td colspan="2">المجمــــــــــــــــوع</td><td id="perf-max-total-${centerKey}">${built.maxTotal}</td><td id="perf-score-total-${centerKey}">0</td></tr></tbody></table>`;
}
...
function renderPerformanceTable(centerKey) {
    ...
    const certificateHTML = `
        <div class="certificate-container" id="performance-certificate-${centerKey}">
            <style>
                #performance-certificate-${centerKey} .workshop-performance-table th { background:#d6ffd6; font-weight:700; }
                #performance-certificate-${centerKey} .workshop-performance-table td { text-align:center; vertical-align:middle; }
                #performance-certificate-${centerKey} .workshop-performance-table .item-text,
                #performance-certificate-${centerKey} .workshop-performance-table .workshop-sub,
                #performance-certificate-${centerKey} .workshop-performance-table .workshop-group { font-weight:600; }
                #performance-certificate-${centerKey} .workshop-performance-table .workshop-group { width:22%; }
                #performance-certificate-${centerKey} .workshop-performance-table .workshop-sub { width:28%; }
            </style>
            ...
            <div class="certificate-header"><h2>جدول ح غ 1</h2><h3>جدول الغرامات الخاصة بمستوى الأداء لموقع ${centerName}</h3><p>عن الفترة من : ${extractStart}م إلى ${extractEnd}م</p></div>
            ${buildPerformanceTableHTML(centerKey)}
...
function saveAllPerformanceScores(centerKey) {
    const inputs = document.querySelectorAll(`#performance-certificate-${centerKey} .score-input`);
    inputs.forEach(input => savePerformanceScoreLive(centerKey, input));
    if (typeof showSuccessMessage === 'function') showSuccessMessage('تم حفظ الدرجات بنجاح!');
}
...
function calculatePerformanceSummary(centerKey) {
    ...
    performanceItems.forEach((item, index) => {
        totalScore += getPerformanceScore(centerKey, item, index);
        maxTotal += parseInt(item.max, 10) || 0;
    });
    const maxEl = document.getElementById(`perf-max-total-${centerKey}`);
    const scoreEl = document.getElementById(`perf-score-total-${centerKey}`);
    ...
    const summaryEl = document.getElementById(`perf-summary-${centerKey}`);
    if (summaryEl) summaryEl.innerHTML = `<p>إجمالي المبلغ لأنشطة القسم = <span>${centerCost.toFixed(2)}</span> ريال</p><p>التقدير الذي حصل عليه المقاول : <span>${percentage.toFixed(2)}%</span></p><p>نسبة الحسم على المقاول لهذا القسم : <span>${deductionPercentage}%</span> ويساوي <span>${deductionAmount.toFixed(2)}</span> ريال</p>`;
...
function saveEditedPerformanceItems(centerKey) {
    ...
    itemRows.forEach(row => {
        ...
        let score = parseInt(row.querySelector('.edit-score-input')?.value, 10);
        if (isNaN(score)) score = max;
        if (!isNaN(max) && max > 0) score = Math.max(0, Math.min(score, max));
        if (text && !isNaN(max) && max > 0) newItems.push({ text, max, score });
    });
    ...
    if (typeof renderAchievementCertificate === 'function') renderAchievementCertificate(centerKey);
    if (typeof showSuccessMessage === 'function') showSuccessMessage('تم حفظ التغييرات على البنود بنجاح!');
}
...
function resetPerformanceScores(centerKey) {
    const allPerformanceData = getPerformanceData();
    delete allPerformanceData[centerKey];
    savePerformanceData(allPerformanceData);
    renderPerformanceTable(centerKey);
...
function buildPrintRows(centerKey) {
    if (isWorkshopCenter(centerKey)) return buildWorkshopPerformanceRows(centerKey, true);
    const items = getPerformanceItems(centerKey);
    let rows = '', maxTotal = 0;
    items.forEach((it, i) => {
        const s = getPerformanceScore(centerKey, it, i);
        rows += `<tr><td>${i + 1}</td><td class="item-text">${it.text}</td><td>${it.max}</td><td>${s}</td></tr>`;
        maxTotal += parseInt(it.max, 10) || 0;
    });
    return { rows, maxTotal };
}
