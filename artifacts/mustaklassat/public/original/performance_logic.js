...
//  تعديل شهادة الأداء للمكاتب الإدارية والورش
//  - المكاتب: جدول ح غ 1 بإجمالي 100 / 87
//  - الورش: جدول ح غ 1 بإجمالي 100 / 90
...
const WORKSHOP_PERFORMANCE_ITEMS = [
    { text: 'المغسلة', max: 6, score: 5 },
    { text: 'السمكرة والبوية', max: 6, score: 6 },
    { text: 'البنشر', max: 6, score: 6 },
    { text: 'التبريد', max: 8, score: 7 },
    { text: 'فحص الأعطال', max: 7, score: 6 },
    { text: 'قسم الكهرباء - تشغيل', max: 10, score: 9 },
    { text: 'قسم الكهرباء - صيانة وقائية', max: 2, score: 2 },
    { text: 'قسم الميكانيكا - تشغيل', max: 16, score: 15 },
    { text: 'قسم الميكانيكا - صيانة وقائية', max: 4, score: 4 },
    { text: 'الأعمال الإدارية', max: 7, score: 6 },
    { text: 'النظافة', max: 7, score: 6 },
    { text: 'المخزن/المشتريات', max: 7, score: 6 },
    { text: 'هل تتم الاستجابة لتعليمات المهندس المشرف من الموقع', max: 5, score: 5 },
    { text: 'هل تتم الاستجابة السريعة لأي حالة طارئة', max: 6, score: 5 },
    { text: 'هل يحمل منسوبو المقاول بطاقات التعريف ويرتدون الزي الرسمي', max: 3, score: 2 }
];

const DEFAULT_PERFORMANCE_ITEMS = ADMIN_OFFICE_PERFORMANCE_ITEMS;
const PERFORMANCE_TEMPLATE_VERSION = 'hg1_admin_workshop_2026_06_22_v1';

function getDefaultPerformanceItems(centerKey) {
    return JSON.parse(JSON.stringify(centerKey === 'admin_staff' ? WORKSHOP_PERFORMANCE_ITEMS : ADMIN_OFFICE_PERFORMANCE_ITEMS));
}
...
function getPerformanceItems(centerKey) {
    const storageKey = `performanceItems_hg1_${centerKey}_v1`;
...
function renderPerformanceTable(centerKey) {
    ...
    performanceItems.forEach((item, index) => {
        ...
        tableRows += `<tr>
            ...
            <td>${item.max}</td>
            <td>
                <input type="number" inputmode="numeric" lang="en" dir="ltr"
                       class="score-input"
                       data-item-index="${index}"
                       min="0" max="${item.max}" value="${score}"
                       oninput="savePerformanceScoreLive('${centerKey}', this)">
            </td>
        </tr>`;
    ...
    const certificateHTML = `
        <div class="certificate-container" id="performance-certificate-${centerKey}">
            <div class="performance-cost-display">
                <span>إجمالي المبلغ لأنشطة القسم (تكلفة العمالة):</span>
                <span class="cost-value">${centerTotalCost.toFixed(2)} ريال</span>
            </div>

            <div class="certificate-header">
                <h2>جدول ح غ 1</h2>
                <h3>جدول الغرامات الخاصة بمستوى الأداء لموقع ${centerName}</h3>
                <p>عن الفترة من : ${extractStart}م إلى ${extractEnd}م</p>
            </div>
            ...
            <table class="performance-table">
                ...
                <tbody>
                    ${tableRows}
                    <tr class="total-row">
                        <td colspan="2">المجمــــــــــــــــوع</td>
                        <td id="perf-max-total-${centerKey}">${maxTotal}</td>
                        <td id="perf-score-total-${centerKey}">0</td>
                    </tr>
                </tbody>
            </table>
...
// تبقى موجودة للتوافق، لكن لم تعد مطلوبة لأن التقدير مفتوح ويحفظ تلقائيًا.
...
function calculatePerformanceSummary(centerKey) {
    ...
    if (summaryEl) {
        summaryEl.innerHTML = `<p>إجمالي المبلغ لأنشطة القسم = <span>${centerCost.toFixed(2)}</span> ريال</p><p>التقدير الذي حصل عليه المقاول : <span>${percentage.toFixed(2)}%</span></p><p>نسبة الحسم على المقاول لهذا القسم : <span>${deductionPercentage}%</span> ويساوي <span>${deductionAmount.toFixed(2)}</span> ريال</p>`;
    }
...
function openEditItemsDialog(centerKey) {
        ...
        itemsHTML += `
            <div class="dialog-item-row" id="edit-item-${index}">
                <div class="item-content">
                    ...
                    <div class="form-group max-score-group"><label>الدرجة القصوى:</label><input type="number" lang="en" dir="ltr" class="edit-max-input" value="${item.max}" min="1"></div>
                    <div class="form-group max-score-group"><label>التقدير الافتراضي:</label><input type="number" lang="en" dir="ltr" class="edit-score-input" value="${getDefaultScore(item)}" min="0" max="${item.max}"></div>
                </div>
    ...
    const content = `
        <div class="dialog-header">
            <h3><i class="fas fa-tasks"></i> إدارة بنود تقييم الأداء لموقع: ${getCenterNames()[centerKey]}</h3>
...
function printPerformanceCertificate(centerKey) {
    ...
    <div class="cert-title">جدول ح غ 1</div>
    <div class="sub-title">جدول الغرامات الخاصة بمستوى الأداء لموقع ${centerName}</div>
    <div class="sub-title">عن الفترة من : ${extractStart}م إلى ${extractEnd}م</div>
    <div class="cost-bar">إجمالي المبلغ لأنشطة القسم (تكلفة العمالة): ${calculateCenterTotalCost(centerKey).toFixed(2)} ريال</div>
    ...
    ${(() => {
        ...
        items.forEach((it, i) => {
            ...
            maxT += (parseInt(it.max, 10) || 0);
            scoreT += s;
        });
        rows += `<tr style="font-weight:bold; background-color: #f0f0f0;"><td colspan="2">المجمــــــــــــــــوع</td><td>${maxT}</td><td>${scoreT}</td></tr>`;
        return rows;
    })()}
    </tbody></table>
    <div class="summary">
    ${(() => {
        ...
        items.forEach((it, i) => {
            maxT += (parseInt(it.max, 10) || 0);
            scoreT += getPerformanceScore(centerKey, it, i);
        });
...
function exportPerformanceToPDF(centerKey) {
    ...
    if (typeof html2pdf !== 'undefined' && element) {
        html2pdf().from(element).save(`performance_${centerKey}.pdf`);
    } else {
        printPerformanceCertificate(centerKey);
    }
}
...
function exportPerformanceToExcel(centerKey) {
    ...
    items.forEach((item, index) => {
        ...
        scoreT += s;
        rows.push([index + 1, item.text, item.max, s]);
    });

    rows.push(['', 'المجمــــــــــــــــوع', maxT, scoreT]);
...
// --- 6. OPEN ADMIN_STAFF PERFORMANCE TAB PATCH ---
// يفتح شهادة الأداء للورش حتى لو كان admin_offices_attendance.js يمنعها.

(function patchAdminStaffPerformanceTab() {
    const oldOpenTab = window.openTab;
    if (typeof oldOpenTab !== 'function') return;

    window.openTab = function(evt, tabName, centerKey) {
        if (!centerKey) {
        ...
        if (tabName === 'performance-tab' && centerKey === 'admin_staff') {
        ...
        return oldOpenTab.call(this, evt, tabName, centerKey);
    };
})();
