...
function getDefaultPerformanceItems(centerKey) {
    return cloneItems(isWorkshopCenter(centerKey) ? WORKSHOP_PERFORMANCE_ITEMS : ADMIN_OFFICE_PERFORMANCE_ITEMS);
}
...
function getPerformanceStorageKey(centerKey) {
    return `performanceItems_hg1_${centerKey}_v4`;
}

function getPerformanceItems(centerKey) {
    // الورش نموذج ثابت لا يقرأ من localStorage نهائياً حتى لا ترجع البنود القديمة.
    if (isWorkshopCenter(centerKey)) {
        localStorage.removeItem('performanceItems_hg1_admin_staff_v1');
        localStorage.removeItem('performanceItems_hg1_admin_staff_v2');
        localStorage.removeItem('performanceItems_hg1_admin_staff_v3');
        localStorage.removeItem('performanceItems_hg1_admin_staff_v1_version');
        localStorage.removeItem('performanceItems_hg1_admin_staff_v2_version');
        localStorage.removeItem('performanceItems_hg1_admin_staff_v3_version');
...
function savePerformanceItems(centerKey, items) {
    if (isWorkshopCenter(centerKey)) return;
    const storageKey = getPerformanceStorageKey(centerKey);
