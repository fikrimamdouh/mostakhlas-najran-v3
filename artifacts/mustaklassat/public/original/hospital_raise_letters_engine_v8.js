...
// Official templates share admin-offices document quality while keeping hospital issuer/data/header. Includes index, page counts, export/import, and collapsible settings.
(function(){
'use strict';
...
var DOC={index:'فهرس مستندات المستخلص',labor:'خطاب العمالة',noPrev:'عدم أسبقية الصرف',salary:'شهادة تسليم الرواتب',vacancies:'بيان الشواغر',vacations:'بيان الإجازات',saudi:'بيان السعودة',final:'خطاب الرفع النهائي',custom:'خطاب مخصص'};
...
function ttl(s,k,t){
  var l=L(s,k),text=P(R(t,s));
  if(l.titleVisible==='no'||!text)return'';
  var ul=l.titleUnderline==='no'?'none':'underline';
  return blk(
    'rl-title',
    '<h1>'+E(text)+'</h1>',
    'text-align:'+E(l.titleAlign||'center')+
    ';font-size:'+N(l.titleFont)+'pt'+
    ';margin-top:'+N(l.titleTop||0)+'mm'+
    ';margin-right:'+N(l.titleRight||0)+'mm'+
    ';width:'+N(l.titleWidth||100)+'%'+
    ';margin-bottom:'+N(l.titleAfter||0)+'mm'+
    ';font-weight:'+N(l.titleWeight||900)+
    ';color:'+E(l.titleColor||'#000')+
    ';text-decoration:'+ul
  );
...
function bodyBlock(s,k,txt,extra){
  var l=L(s,k);
  var text=String(R(txt,s)||'').trim();
  if(l.bodyVisible==='no'||!text)return'';

  var col=extra==='close'?(l.closeColor||l.bodyColor):(l.bodyColor||'#000');

  return blk(
    'rl-body '+(extra||''),
    E(text).replace(/\n/g,'<br>'),
    'text-align:'+E(l.bodyAlign||'justify')+
    ';font-size:'+N(l.bodyFont||15)+'pt'+
    ';line-height:'+N(l.bodyLine||2.1)+
    ';width:'+N(l.bodyWidth||100)+'%'+
    ';margin-right:'+N(l.bodyRight||0)+'mm'+
    ';margin-bottom:'+N(l.bodyAfter||0)+'mm'+
    ';font-weight:'+N(l.bodyWeight||700)+
    ';color:'+E(col)
  );
}

function closeBlock(s,k,txt){
  var l=L(s,k);
  var text=String(R(txt,s)||'').trim();
  if(l.closeVisible==='no'||!text)return'';
  return bodyBlock(s,k,text+'\nوتقبلوا تحياتنا ،،','close');
}
...
function sig(s,k,items){
  ...
  for(var i=1;i<=3;i++){
    var t=R(P(x['sign'+i+'Title']),s).trim();
    var n=R(P(x['sign'+i+'Name']),s).trim();
    if(t||n)out.push({t:t,n:n});
  }

  if(!out.length && items && items.length){
    out=items.filter(function(it){return P(it.t,it.n)});
  }

  if(!out.length)return'';

  out=out.slice(0,Math.max(1,Math.min(3,N(l.signatureCount)||out.length||1)));

  var cols=Math.max(1,Math.min(3,N(l.signatureColumns)||1));

  return'<div class="rl-signatures" style="margin-top:'+N(l.sigTop||24)+'mm;grid-template-columns:repeat('+cols+',1fr);font-size:'+N(l.signatureFont||14)+'pt;text-align:'+E(l.signatureAlign||'center')+';font-weight:'+N(l.signatureWeight||800)+';gap:'+N(l.signatureGap||16)+'mm;color:'+E(l.signatureColor||'#000')+'">'+
    out.map(function(x){
      var line=l.signatureLine==='no'?'':'<div class="sig-line"></div>';
      return'<div class="sig-cell"><b>'+E(x.t||'')+'</b>'+line+'<b>'+E(x.n||'')+'</b></div>';
    }).join('')+
  '</div>';
...
function indexItems(s){return[['final','خطاب الرفع النهائي'],['labor','خطاب العمالة'],['noPrev','عدم أسبقية الصرف'],['salary','شهادة تسليم الرواتب'],['vacancies','بيان الشواغر'],['vacations','بيان الإجازات'],['saudi','بيان السعودة']]}
function pageCount(s,k){return Math.max(1,N(s['pageCount_'+k])||1)}
function indexDoc(s){var k='index',x=s.letters[k]||{},page=Math.max(1,N(s.indexStartPage)||1),rows=[];indexItems(s).forEach(function(it,i){var c=pageCount(s,it[0]);rows.push('<tr><td>'+an(i+1)+'</td><td>'+E(it[1])+'</td><td>'+an(page)+'</td><td>'+an(c)+'</td></tr>');page+=c});return pg(s,k,lead(s,k,x)+bodyBlock(s,k,x.body)+tb(s,k,['م','اسم المستند','صفحة البداية','عدد الصفحات'],rows,'')+closeBlock(s,k,x.close)+sig(s,k)+stamp(s,k)+noteBlock(s,k))}
function build(k,s){if(k==='packet')return['index','final','labor','noPrev','salary','vacancies','vacations','saudi'].map(function(q){return build(q,s)}).join('');return k==='index'?indexDoc(s):k==='labor'?labor(s):k==='noPrev'?noPrev(s):k==='salary'?salary(s):k==='vacancies'?vacancies(s):k==='vacations'?vacations(s):k==='saudi'?saudi(s):k==='final'?finalDoc(s):pg(s,'custom',lead(s,'custom',s.letters.custom)+bodyBlock(s,'custom',(s.customBody||s.letters.custom.body).replace(/\n/g,'<br>'))+closeBlock(s,'custom',s.letters.custom.close)+sig(s,'custom')+stamp(s,'custom')+noteBlock(s,'custom'))}
...
function ED(s,k){
  var x=s.letters[k],l=L(s,k);

  return '<details id="rl-doc-settings" class="settings-panel doc-panel" data-panel="doc" '+((s.uiOpen&&s.uiOpen.doc)?'open':'')+'>'+
    '<summary>إعدادات الخطاب المختار</summary>'+
    '<div class="rl-note">أي تعديل أو حذف يظهر فوراً في المعاينة ويحفظ تلقائياً.</div>'+
    '<div class="rl-grid">'+

    sectionTitle('النصوص الثابتة')+
    F('letters.'+k+'.recipient','المخاطب',x.recipient)+
    F('letters.'+k+'.suffix','الصفة',x.suffix)+
    F('letters.'+k+'.title','الموضوع',x.title)+
    A('letters.'+k+'.body','جسم الخطاب',x.body)+
    A('letters.'+k+'.close','الخاتمة',x.close)+
    A('letters.'+k+'.note','عبارة ختامية / ملاحظة',x.note)+

    sectionTitle('الموضوع والمخاطب والتحية')+
    yesNo('layout.'+k+'.recipientVisible','إظهار المخاطب',l.recipientVisible)+
    yesNo('layout.'+k+'.titleVisible','إظهار الموضوع',l.titleVisible)+
    yesNo('layout.'+k+'.titleFirst','الموضوع فوق الكل',l.titleFirst)+
    yesNo('layout.'+k+'.titleUnderline','خط تحت الموضوع',l.titleUnderline)+
    yesNo('layout.'+k+'.recipientSpread','فصل المخاطب والصفة',l.recipientSpread)+
    yesNo('layout.'+k+'.greetingVisible','إظهار التحية',l.greetingVisible)+
    F('layout.'+k+'.greetingText','نص التحية',l.greetingText)+

    F('layout.'+k+'.top','نزول الخطاب داخل الصفحة mm',l.top,'number')+
    F('layout.'+k+'.right','تحريك الخطاب من اليمين mm',l.right,'number')+
    F('layout.'+k+'.width','عرض مساحة الخطاب mm',l.width,'number')+

    S('layout.'+k+'.titleAlign','محاذاة الموضوع',l.titleAlign,[['right','يمين'],['center','وسط'],['left','يسار']])+
    F('layout.'+k+'.titleFont','حجم خط الموضوع',l.titleFont,'number')+
    F('layout.'+k+'.titleAfter','مسافة بعد الموضوع mm',l.titleAfter,'number')+

    F('layout.'+k+'.recipientFont','حجم خط المخاطب',l.recipientFont,'number')+
    S('layout.'+k+'.recipientAlign','محاذاة المخاطب',l.recipientAlign,[['right','يمين'],['center','وسط'],['left','يسار']])+

    F('layout.'+k+'.greetingFont','حجم خط التحية',l.greetingFont,'number')+
    S('layout.'+k+'.greetingAlign','محاذاة التحية',l.greetingAlign,[['right','يمين'],['center','وسط'],['left','يسار']])+
    F('layout.'+k+'.greetingAfter','مسافة بعد التحية mm',l.greetingAfter,'number')+

    sectionTitle('جسم الخطاب والجداول والتوقيعات')+
    yesNo('layout.'+k+'.bodyVisible','إظهار جسم الخطاب',l.bodyVisible)+
    yesNo('layout.'+k+'.closeVisible','إظهار الخاتمة',l.closeVisible)+
    yesNo('layout.'+k+'.noteVisible','إظهار الملاحظة',l.noteVisible)+
    F('layout.'+k+'.bodyFont','حجم خط جسم الخطاب',l.bodyFont,'number')+
    aligns('layout.'+k+'.bodyAlign','محاذاة جسم الخطاب',l.bodyAlign)+
    F('layout.'+k+'.bodyLine','تباعد سطور الجسم',l.bodyLine,'number')+

    F('layout.'+k+'.tableTop','مسافة قبل الجدول mm',l.tableTop,'number')+
    F('layout.'+k+'.tableWidth','عرض الجدول mm',l.tableWidth,'number')+
    F('layout.'+k+'.tableFont','حجم خط الجدول',l.tableFont,'number')+
    F('layout.'+k+'.tableRowHeight','ارتفاع الصف mm',l.tableRowHeight,'number')+

    yesNo('layout.'+k+'.signatureEnabled','إظهار التواقيع',l.signatureEnabled)+
    yesNo('layout.'+k+'.signatureLine','خط التوقيع',l.signatureLine)+
    F('layout.'+k+'.signatureCount','عدد التوقيعات',l.signatureCount,'number')+
    F('layout.'+k+'.signatureColumns','عدد أعمدة التواقيع',l.signatureColumns,'number')+
    F('layout.'+k+'.sigTop','مكان التواقيع من أعلى mm',l.sigTop,'number')+
    F('layout.'+k+'.signatureFont','حجم خط التواقيع',l.signatureFont,'number')+

    F('letters.'+k+'.sign1Title','صفة التوقيع 1',x.sign1Title)+
    F('letters.'+k+'.sign1Name','اسم التوقيع 1',x.sign1Name)+
    F('letters.'+k+'.sign2Title','صفة التوقيع 2',x.sign2Title)+
    F('letters.'+k+'.sign2Name','اسم التوقيع 2',x.sign2Name)+
    F('letters.'+k+'.sign3Title','صفة التوقيع 3',x.sign3Title)+
    F('letters.'+k+'.sign3Name','اسم التوقيع 3',x.sign3Name)+

    yesNo('layout.'+k+'.stampVisible','إظهار الختم',l.stampVisible)+
    F('layout.'+k+'.stampTop','مكان الختم من أعلى mm',l.stampTop,'number')+
...
function bindPanels(scope){scope.querySelectorAll('details[data-panel]').forEach(function(d){d.ontoggle=function(){var s=READUI();s.uiOpen=s.uiOpen||{};s.uiOpen[d.dataset.panel]=d.open;s[VISUAL_MARK]=true;s[TEMPLATE_MARK]=true;SV(s)}})}
...
function switchDoc(){var ss=READUI(),box=document.getElementById('rl-doc-settings-wrap');ss[VISUAL_MARK]=true;ss[TEMPLATE_MARK]=true;SV(ss);if(box){box.innerHTML=ED(ss,ss.selectedDocKey||'labor');bindAuto(box);bindPanels(box)}PV(ss,true);flash('تم اختيار الخطاب')}
...
function MT(){
  ...
  root.innerHTML=
    ...
    '<div class="rl-shell">'+
      ...
      '<section class="rl-settings">'+
        generalBox(s,h)+
        '<details class="settings-panel select-panel" data-panel="select" '+((s.uiOpen&&s.uiOpen.select)?'open':'')+'><summary>اختيار الخطاب</summary>'+DOCSEL(s)+'</details>'+  
      ...
      '<div class="printbar">'+
        ...
        '<button class="rl-btn safe" id="export-settings" type="button">تصدير الإعدادات</button>'+  
        '<button class="rl-btn safe" id="import-settings-btn" type="button">استيراد الإعدادات</button>'+  
        '<input id="import-settings-file" type="file" accept="application/json" style="display:none">'+  
        ...
        '<button class="rl-btn" data-print="index">طباعة الفهرس</button>'+  
  ...
  root.querySelector('#export-settings').onclick=exportSettings;
  root.querySelector('#import-settings-btn').onclick=function(){root.querySelector('#import-settings-file').click()};
  root.querySelector('#import-settings-file').onchange=function(){importSettings(this.files&&this.files[0])};
  ...
  PV(s,true);
  console.info('[HospitalLettersEngine] V8 mounted office-quality documents v14 + index/settings');
}
function generalBox(s,h){
  return '<details class="settings-panel global-panel" data-panel="global" '+((s.uiOpen&&s.uiOpen.global)?'open':'')+'>'+  
    '<summary>الإعدادات العامة والترويسة</summary>'+  
    '<div class="rl-note">أي تعديل هنا يحفظ تلقائياً ويظهر في المعاينة مباشرة.</div>'+  
    '<div class="rl-grid">'+
      F('hospitalName','اسم المستشفى',s.hospitalName)+
      F('companyName','الشركة',s.companyName)+
      F('contractName','العقد',s.contractName)+
      F('issuerName','الجهة المصدرة',s.issuerName)+
      F('issuerDepartment','الإدارة / القسم المصدر',s.issuerDepartment)+
      F('issuerLocation','المدينة',s.issuerLocation)+
      F('issuerSignatureTitle','صفة توقيع الجهة المصدرة',s.issuerSignatureTitle)+
      F('issuerSignatureName','اسم توقيع الجهة المصدرة',s.issuerSignatureName)+
      F('managerTitle','صفة المدير',s.managerTitle)+
      F('managerName','اسم المدير',s.managerName)+
      F('accountantTitle','صفة المحاسب',s.accountantTitle)+
      F('accountantName','اسم المحاسب',s.accountantName)+
      F('vatRate','الضريبة %',s.vatRate,'number')+
      F('manualGrand','مبلغ يدوي شامل الضريبة',s.manualGrand,'number')+
      F('requiredSaudi','نسبة السعودة المطلوبة %',s.requiredSaudi,'number')+
      A('customBody','نص الخطاب المخصص',s.customBody)+
      '<label class="rl-field check"><span>تفعيل الترويسة</span><input data-f="letterheadEnabled" type="checkbox" '+(s.letterheadEnabled?'checked':'')+'></label>'+  
      S('letterheadMode','طريقة الترويسة',s.letterheadMode,[['external','مطبوع خارجي — لا تطبع صورة وتُنزل المحتوى فقط'],['full','صورة A4 كاملة كخلفية'],['top','صورة ترويسة علوية فقط']])+
      F('letterheadHeightMm','ارتفاع الترويسة العلوية — ملم',s.letterheadHeightMm,'number')+
      F('contentTopMm','نزول بداية الخطاب من أعلى الورقة — ملم',s.contentTopMm,'number')+
      F('blankLines','نزول إضافي بعدد السطور',s.blankLines,'number')+
      F('lineHeightMm','ارتفاع السطر بالملليمتر',s.lineHeightMm,'number')+
      F('printScale','تكبير/تصغير الطباعة %',s.printScale,'number')+
      sectionTitle('الفهرس وعدد الصفحات')+
      F('indexStartPage','رقم بداية الفهرس',s.indexStartPage,'number')+
      F('pageCount_final','صفحات خطاب الرفع النهائي',s.pageCount_final,'number')+
      F('pageCount_labor','صفحات خطاب العمالة',s.pageCount_labor,'number')+
      F('pageCount_noPrev','صفحات عدم أسبقية الصرف',s.pageCount_noPrev,'number')+
      F('pageCount_salary','صفحات شهادة الرواتب',s.pageCount_salary,'number')+
      F('pageCount_vacancies','صفحات الشواغر',s.pageCount_vacancies,'number')+
      F('pageCount_vacations','صفحات الإجازات',s.pageCount_vacations,'number')+
      F('pageCount_saudi','صفحات السعودة',s.pageCount_saudi,'number')+
      F('pageCount_custom','صفحات الخطاب المخصص',s.pageCount_custom,'number')+
    '</div>'+  
    '<div class="head-status">حالة الترويسة: '+(h?'محفوظة':'لا توجد ترويسة محفوظة')+'</div>'+  
    '<div class="rl-row"><input id="head-file" type="file" accept="image/*"><button class="rl-btn danger" id="delete-head" type="button">حذف الترويسة</button></div>'+  
  '</details>';
}function uiCSS(h){
