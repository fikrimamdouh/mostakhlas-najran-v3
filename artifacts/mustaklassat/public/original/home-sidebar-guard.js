...
function openLetters(e){if(ON_LETTERS_PAGE)return;if(e){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation()}console.warn('[HospitalLetters] open standalone:',TARGET);location.href=TARGET}
function routeLetters(){if(ON_LETTERS_PAGE)return;try{Array.prototype.slice.call(document.querySelectorAll('a,button,[onclick],[data-href]')).forEach(function(el){if(!isLetters(el))return;el.setAttribute('data-hospital-letters-route','standalone');if(el.tagName==='A')el.setAttribute('href',TARGET);if(el.hasAttribute&&el.hasAttribute('data-href'))el.setAttribute('data-href',TARGET)})}catch(e){}}
...
document.addEventListener('click',function(e){if(ON_LETTERS_PAGE)return;var t=e.target&&e.target.closest?e.target.closest('a,button,[onclick],[data-href]'):null;if(!t)return;if((t.closest&&t.closest('[data-hospital-letters-route="standalone"]'))||isLetters(t))return openLetters(e)},true);
