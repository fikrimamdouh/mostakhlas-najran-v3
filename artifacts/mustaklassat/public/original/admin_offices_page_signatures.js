...
(function () {
  ...
  if (window.__ADMIN_OFFICES_PAGE_SIGNATURES_CONTROLS_SEPARATED_V8__) return;
  window.__ADMIN_OFFICES_PAGE_SIGNATURES_CONTROLS_SEPARATED_V8__ = true;
  ...
  function normalizePrefs(prefs) {
    ...
    return {
      ...
      align: ['right', 'center', 'left'].indexOf(prefs.align) > -1 ? prefs.align : 'center',
      size: ['small', 'normal', 'large'].indexOf(prefs.size) > -1 ? prefs.size : 'normal',
      width: Math.max(120, Math.min(320, Number(prefs.width) || 180))
    };
  }
  ...
  function currentCenterKey() {
    ...
    for (var i = 0; i < keys.length; i++) if (names[keys[i]] && title.indexOf(names[keys[i]]) > -1) return keys[i];
  ...
  function buildGrandSignaturesHtml() {
    ensureGrandDefaults();
    var prefs = normalizePrefs(readJson(PREF_PREFIX + GRAND_KEY, {}));
    ...
    return '<section class="sign grand-signatures grand-align-' + prefs.align + ' grand-size-' + prefs.size + '" data-source="admin_offices_page_signatures_v8" style="--grand-item-width:' + prefs.width + 'px">' + rows.map(function (s) {
      return '<div class="grand-signature-item"><div class="g-title">' + esc(s.title || '') + '</div><div class="g-name">' + esc(s.name || '') + '</div></div>';
    }).join('') + '</section>';
  }

  function injectGrandStyle(doc) {
    if (!doc || doc.getElementById('admin-offices-grand-signatures-v8-style')) return;
    var style = doc.createElement('style');
    style.id = 'admin-offices-grand-signatures-v8-style';
    style.textContent = '' +
      '.grand-signatures{display:flex;flex-wrap:wrap;direction:ltr;gap:18px;margin-top:28px;width:100%;align-items:flex-start}' +
      '.grand-signatures.grand-align-right{justify-content:flex-end}.grand-signatures.grand-align-center{justify-content:center}.grand-signatures.grand-align-left{justify-content:flex-start}' +
      '.grand-signatures .grand-signature-item{direction:rtl;text-align:center;width:var(--grand-item-width,180px);page-break-inside:avoid;break-inside:avoid}' +
      '.grand-signatures .g-title{font-weight:900;color:#111827;font-size:15px;line-height:1.45;margin-bottom:4px}' +
      '.grand-signatures .g-name{font-weight:800;color:#334155;font-size:13px;line-height:1.45;min-height:16px}' +
      '.grand-signatures .line{display:none!important}' +
      '.grand-size-small{gap:10px}.grand-size-small .g-title{font-size:13px}.grand-size-small .g-name{font-size:11.5px}' +
      '.grand-size-large{gap:22px}.grand-size-large .g-title{font-size:18px}.grand-size-large .g-name{font-size:15px}' +
      '@media print{.grand-signatures .g-title{color:#000}.grand-signatures .g-name{color:#000}}';
  ...
  function applyGrandSignaturesToWindow(win) {
    try {
      ...
      cert.querySelectorAll('.grand-signatures, .cert > .sign').forEach(function (el) { try { el.remove(); } catch (_) {} });
  ...
  function patchWindowOpenForGrandSignatures() {
    if (window.open.__adminOfficesGrandSignatureInjectV8) return;
    ...
    window.open.__adminOfficesGrandSignatureInjectV8 = true;
  }
  ...
  function removeOfficePageSignatureControlsNow() {
    ...
    cleanupRunning = true;
    try {
      document.querySelectorAll('.admin-page-signature-bar, .admin-page-signature-block, #sb-container-admin_offices').forEach(function (el) { try { el.remove(); } catch (_) {} });
      document.querySelectorAll('[id^="attendance-signatures-"], [id^="perf-signatures-"], [id^="ach-signatures-"]').forEach(function (el) { el.dataset.adminOfficesNoInlineSigs = '1'; if (el.innerHTML) el.innerHTML = ''; });
      var details = document.getElementById('center-details-view');
      if (details) details.querySelectorAll('button').forEach(function (btn) { var text = String(btn.textContent || '').replace(/\s+/g, ' ').trim(); if (/تعديل التواقيع|تعديل التوقيع|إضافة توقيع|نسخ للمكاتب/.test(text)) { try { btn.remove(); } catch (_) {} } });
  ...
  function ensureGrandButton(attempt) {
    ...
    if (!bar) { if ((attempt || 0) < 20) setTimeout(function () { ensureGrandButton((attempt || 0) + 1); }, 500); return; }
    ...
    if (!btn) {
      ...
      var certBtn = Array.prototype.slice.call(bar.querySelectorAll('button')).find(function (b) { return String(b.textContent || '').indexOf('الشهادة الإجمالية') > -1 && b.id !== GRAND_BTN_ID; });
      if (certBtn && certBtn.nextSibling) bar.insertBefore(btn, certBtn.nextSibling); else bar.appendChild(btn);
    }
  ...
  window.AdminOfficesPageSignatures = { signatureKey: signatureKey, currentCenterKey: currentCenterKey, refresh: scheduleOfficeSignatureCleanup, openGrand: openGrandSignatureDialog, buildGrandSignaturesHtml: buildGrandSignaturesHtml, applyGrandSignaturesToWindow: applyGrandSignaturesToWindow };
  ...
  console.info('[Admin Offices Signatures] v8: grand signature layout prefs applied');
})();