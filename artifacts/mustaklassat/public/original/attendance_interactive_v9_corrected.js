...
function createAttendanceSelect(currentStatus, departmentKey, employeeIndex, dayIndex) {
  const statuses = [
    ...
    { code: 'ب', name: 'قبل بدء العقد' },
    { code: 'ن', name: 'بعد نهاية العقد' }
  ];
...
function updateEmployeeAttendance(departmentKey, employeeIndex, dayIndex, newStatus) {
    try {
        ...
        if (select) {
            ...
            select.classList.remove('present', 'absent', 'leave', 'vacant', 'pending', 'pre-contract', 'post-contract');
...
cells.push(`
    <td>
        <!-- هذا الحقل سيظهر في العرض العادي ويختفي عند الطباعة -->
        <input type="text" value="${emp.iqamaId || ''}" 
               ...
               style="width: 120px; text-align: center; border: 1px solid #ccc; border-radius: 4px;">
        
        <!-- هذا النص سيظهر فقط عند الطباعة -->
        <span class="print-only" style="display: none; font-weight: bold;">${emp.iqamaId || ''}</span>
    </td>
`);
...
function editEmployeeAdvanced(departmentKey, employeeIndex) {
  try {
    ...
    editDialog.innerHTML = `
      <h3 style="margin-top: 0; color: #333;">تعديل بيانات الموظف</h3>
      <div style="margin-bottom: 10px;">
        <label style="display: block; margin-bottom: 5px;">مسمى الوظيفة:</label>
        <input type="text" id="edit-job-title" value="${employee.jobTitle || ''}" style="width: 100%; padding: 5px; border: 1px solid #ccc; border-radius: 4px;">
      </div>
      <div style="margin-bottom: 10px;">
        <label style="display: block; margin-bottom: 5px;">اسم الموظف:</label>
        <input type="text" id="edit-name" value="${employee.name || ''}" style="width: 100%; padding: 5px; border: 1px solid #ccc; border-radius: 4px;">
      </div>
      <div style="margin-bottom: 10px;">
        <label style="display: block; margin-bottom: 5px;">الراتب الشهري:</label>
        <input type="number" id="edit-salary" value="${employee.salary || 0}" style="width: 100%; padding: 5px; border: 1px solid #ccc; border-radius: 4px;">
      </div>
      <div style="margin-bottom: 10px;">
        <label style="display: block; margin-bottom: 5px;">الفئة:</label>
        <select id="edit-category" style="width: 100%; padding: 5px; border: 1px solid #ccc; border-radius: 4px;">
          <option value="1" ${employee.category === '1' ? 'selected' : ''}>1</option>
          <option value="2" ${employee.category === '2' ? 'selected' : ''}>2</option>
          <option value="3" ${employee.category === '3' ? 'selected' : ''}>3</option>
          <option value="4" ${employee.category === '4' ? 'selected' : ''}>4</option>
          <option value="5" ${employee.category === '5' ? 'selected' : ''}>5</option>
          <option value="6" ${employee.category === '6' ? 'selected' : ''}>6</option>
          <option value="7" ${employee.category === '7' ? 'selected' : ''}>7</option>
        </select>
      </div>
      <div style="margin-bottom: 15px;">
        <label style="display: block; margin-bottom: 5px;">الجنسية:</label>
        <select id="edit-nationality" style="width: 100%; padding: 5px; border: 1px solid #ccc; border-radius: 4px;">
          <option value="سعودي" ${employee.nationality === 'سعودي' ? 'selected' : ''}>سعودي</option>
          <option value="مصري" ${employee.nationality === 'مصري' ? 'selected' : ''}>مصري</option>
          <option value="هندي" ${employee.nationality === 'هندي' ? 'selected' : ''}>هندي</option>
          <option value="باكستاني" ${employee.nationality === 'باكستاني' ? 'selected' : ''}>باكستاني</option>
          <option value="فلبيني" ${employee.nationality === 'فلبيني' ? 'selected' : ''}>فلبيني</option>
          <option value="بنجلادش" ${employee.nationality === 'بنجلادش' ? 'selected' : ''}>بنجلادش</option>
          <option value="أخرى" ${employee.nationality === 'أخرى' ? 'selected' : ''}>أخرى</option>
        </select>
      </div>
      <div style="text-align: center;">
        <button onclick="saveEmployeeEdit('${departmentKey}', ${employeeIndex})" 
...
function openPDFExportDialog() {
  // إنشاء نافذة اختيار الأقسام لتصدير PDF
    ...
    departmentCheckboxes += `
      <div style="margin-bottom: 8px;">
        <label style="display: flex; align-items: center; cursor: pointer;">
          <input type="checkbox" value="${dept.key}" checked style="margin-left: 8px;">
          ${dept.name}
        </label>
      </div>
    `;
  ...
  exportDialog.innerHTML = `
    <h3 style="margin-top: 0; color: #007bff; text-align: center;">تصدير PDF</h3>
    
    <div style="margin-bottom: 15px;">
      <label style="display: flex; align-items: center; margin-bottom: 10px; cursor: pointer;">
        <input type="checkbox" id="select-all-pdf" checked onchange="toggleAllDepartmentsPDF(this)" style="margin-left: 8px;">
        <strong>تحديد جميع الأقسام</strong>
      </label>
    </div>
    ...
    <div style="text-align: center; margin-top: 15px;">
      <button onclick="exportSelectedDepartmentsToPDF()" 
...
const departments = [
    ...
    { key: 'civil_works', name: getDepartmentName('civil_works') }, // <-- تم التصحيح
    { key: 'mechanical', name: getDepartmentName('mechanical') },
    { key: 'laundry', name: getDepartmentName('laundry') },
    { key: 'patient_services', name: getDepartmentName('patient_services') }, // <-- تم التصحيح
  ...
  exportDialog.innerHTML = `
    <h3 style="margin-top: 0; color: #28a745; text-align: center;">تصدير Excel</h3>
    
    <div style="margin-bottom: 15px;">
      <label style="display: flex; align-items: center; margin-bottom: 10px; cursor: pointer;">
        <input type="checkbox" id="select-all-excel" checked onchange="toggleAllDepartmentsExcel(this)" style="margin-left: 8px;">
        <strong>تحديد جميع الأقسام</strong>
      </label>
    </div>
    ...
    <div style="text-align: center; margin-top: 15px;">
      <button onclick="exportSelectedDepartmentsToExcel()" 
      ...
      <button onclick="closeExcelExportDialog()" 
              style="background: #6c757d; color: white; border: none; padding: 10px 20px; margin: 5px; border-radius: 4px; cursor: pointer; font-size: 14px;">
        إلغاء
      </button>
    </div>
  ...
  overlay.style.cssText = `
    ...
    z-index: 999;
  `;
  overlay.id = 'excel-export-overlay';
            ...
            employees.forEach((emp, index) => {
                try { // <--- بداية بلوك الحماية
                    ...
                    days.slice(0, daysInMonth).forEach(day => {
                        ...
                        else if (day !== 'ب' && day !== 'ن') deductionOnlyDays++;
                    });
...
const currentMonthYear = extractStartDate.toLocaleDateString('ar-EG', { month: 'long', year: 'numeric', calendar: 'gregory' });
  ...
  departmentTables.forEach((tableDiv, idx) => {
    ...
    if (idx > 0) doc.write(`<div class="print-break"></div>`);
    ...
    if (table) {
      let tableHTML = table.tHead ? table.tHead.outerHTML : '';
      let tbodyHTML = '<tbody>';
      table.querySelectorAll('tbody tr').forEach(row => {
        ...
        row.querySelectorAll('td').forEach(td => {
          ...
          if (select) {
            ...
            tbodyHTML += `<td>${td.textContent}</td>`;
          }
        });
        tbodyHTML += '</tr>';
      });
...
// ✅✅✅ أضف هذا الكود هنا ✅✅✅
  const grandTotalHTML = document.querySelector('.grand-total')?.outerHTML;
  if (grandTotalHTML) {
    doc.write(grandTotalHTML);
  }
// ===================================================================
//      ✅✅✅ كود التواقيع النهائي الذي يقرأ من المصدر الصحيح ✅✅✅
// ===================================================================

// 1. اقرأ أحدث البيانات من المصدر الصحيح (التواقيع الديناميكية)
const latestSignatures = getSignatures(); // هذه الدالة لديك بالفعل وهي تقرأ من 'dynamicSignatures'

// 2. بناء كود HTML للتواقيع
let signaturesHTML = `
...
if (latestSignatures.length > 0) {
    ...
    signaturesHTML += '<p>لا توجد تواقيع معتمدة.</p>';
}
...
// 3. كتابة كود التواقيع الصحيح في نافذة الطباعة
...
// باقي الكود هيفضل زي ما هو حتى قسم "دوال تصدير PDF و Excel"
// حذف دوال تصدير PDF من قسم "دوال تصدير PDF و Excel"
// القسم الجديد هيبقى بس لتصدير Excel:

// ===== دوال تصدير Excel مع خيار اختيار الأقسام =====
...
function openExcelExportDialog() {
 ...
 // استبدل المصفوفة القديمة بهذه النسخة الصحيحة
const departments = [
    { key: 'cleaning', name: getDepartmentName('cleaning') },
    { key: 'electricity', name: getDepartmentName('electricity') },
    { key: 'agriculture', name: getDepartmentName('agriculture') },
    { key: 'civil_works', name: getDepartmentName('civil_works') }, // <-- تأكد من أنها بالشرطة السفلية
    { key: 'mechanical', name: getDepartmentName('mechanical') },
    { key: 'laundry', name: getDepartmentName('laundry') },
    { key: 'patient_services', name: getDepartmentName('patient_services') }, // <-- تأكد من أنها بالشرطة السفلية
    { key: 'admin_saudi', name: getDepartmentName('admin_saudi') }
];
...
function openUpdateSignaturesDialog() {
  ...
  // جلب التواقيع الحالية من localStorage أو HTML
  let contractData = {};
  try {
    contractData = JSON.parse(localStorage.getItem('persistentContractData') || '{}');
  } catch (error) {
    console.warn('خطأ في جلب بيانات التواقيع:', error);
  }
  ...
  updateDialog.innerHTML = `
    <h3 style="margin-top: 0; color: #ffca2c; text-align: center;">تحديث التواقيع</h3>
    <div style="margin-bottom: 10px;">
      <label style="display: block; margin-bottom: 5px; font-weight: bold;">مدير المشروع:</label>
      <input type="text" id="update-engineering-manager" value="${engineeringManager}" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
    </div>
    <div style="margin-bottom: 10px;">
      <label style="display: block; margin-bottom: 5px; font-weight: bold;">رئيس قسم الصيانة:</label>
      <input type="text" id="update-general-services-manager" value="${generalServicesManager}" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
    </div>
    <div style="margin-bottom: 10px;">
      <label style="display: block; margin-bottom: 5px; font-weight: bold;">مدير المستشفى:</label>
      <input type="text" id="update-hospital-manager" value="${hospitalManager}" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
    </div>
    <div style="margin-bottom: 10px;">
      <label style="display: block; margin-bottom: 5px; font-weight: bold;">مندوب المقاول:</label>
      <input type="text" id="update-follow-up-manager" value="${followUpManager}" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
    </div>
    <div style="text-align: center; margin-top: 15px;">
      <button onclick="saveUpdateSignatures()" 
              style="background: #ffca2c; color: black; border: none; padding: 10px 20px; margin: 5px; border-radius: 4px; cursor: pointer; font-size: 14px;">
        حفظ التغييرات
      </button>
      <button onclick="closeDialog('update-signatures-dialog')" 
              style="background: #6c757d; color: white; border: none; padding: 10px 20px; margin: 5px; border-radius: 4px; cursor: pointer; font-size: 14px;">
        إلغاء
      </button>
    </div>
  ...
  overlay.style.cssText = `
    ...
    z-index: 999;
  `;
  overlay.id = 'update-signatures-overlay';
