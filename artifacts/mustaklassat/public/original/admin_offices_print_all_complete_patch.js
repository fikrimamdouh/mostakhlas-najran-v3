...
// - الحضور والانصراف: من نفس جدول الصفحة الأصلي وإعداداته، وليس جدول مبني من الصفر.
...
(function () {
  ...
  function period() { try { if (typeof window.getExtractPeriodDetails === 'function') return window.getExtractPeriodDetails() || {}; } catch (_) {} var e=readJson('persistentExtractData',{}), s=new Date(e.extractStart||Date.now()), n=new Date(e.extractEnd||e.extractStart||Date.now()); return {startDate:s,daysInExtract:Math.max(1,Math.ceil((n-s)/86400000)+1||30),totalDaysInMonth:new Date(s.getFullYear(),s.getMonth()+1,0).getDate()||30}; }
  function contract() { return readJson('persistentContractData', {}); }
  function extract() { return readJson('persistentExtractData', {}); }

  function statusInfo(code) { try { if (window.STATUS_CODES && window.STATUS_CODES[code]) return window.STATUS_CODES[code]; } catch (_) {} return ({'ح':{isAbsence:false},'غ':{isAbsence:true},'ج':{isAbsence:false,isSpecial:true},'ش':{isAbsence:false,isSpecial:true},'ت':{isAbsence:false,isSpecial:true},'ب':{isAbsence:false,isSpecial:true},'ن':{isAbsence:false,isSpecial:true}})[code] || {isAbsence:false}; }
  ...
  function signaturesHtml(type, key, itemClass) {
    ...
    if (itemClass === 'signature-item') return '<div class="signatures-grid">' + sigs.map(function(s){ return '<div class="signature-item"><div class="title">'+esc(s.title||'')+'</div><div class="line"></div><div class="name">'+esc(s.name||'................')+'</div></div>'; }).join('') + '</div>';
  ...
  function cloneOriginalAttendanceTable(key) {
    ...
    try {
      window.renderAttendanceTable(key);
      var built = document.getElementById('table-div-' + key);
      return built ? built.cloneNode(true) : null;
    } finally {
      tab.innerHTML = oldHtml;
    }
  }
  ...
  function cleanClonedAttendance(clone, key) {
    ...
    if (sigContainer) sigContainer.innerHTML = signaturesHtml('attendance', key, 'signature-item');
    else clone.insertAdjacentHTML('beforeend', signaturesHtml('attendance', key, 'signature-item'));
  ...
  function fallbackAttendancePage(key) {
    ...
    rows.forEach(function(emp,idx){var r=employeeCalc(emp,p);totalCost+=num(r.costForPeriod);totalDed+=num(r.deduction);totalFine+=num(r.absenceFine)+num(r.nationalityFine);totalNet+=num(r.netSalary);var dayCells=(r.days||[]).map(function(d){return '<td class="day-cell">'+esc(d)+'</td>';}).join('');body+='<tr><td>'+(idx+1)+'</td><td class="job-title">'+esc(emp.jobTitle||emp.position||'')+'</td><td>'+esc(emp.category||'')+'</td><td class="employee-name">'+esc(emp.name||'')+'</td>'+dayCells+'<td>'+money(r.costForPeriod)+'</td><td>'+num(r.attendanceDays)+'</td><td>'+num(r.absenceDays)+'</td><td>'+money(r.deduction)+'</td><td>'+money(r.absenceFine)+'</td><td>'+money(r.netSalary)+'</td><td>'+esc(emp.nationality||'')+'</td><td>'+money(r.nationalityFine)+'</td><td>'+esc(emp.iqamaId||emp.idNumber||'')+'</td></tr>';});
    ...
    var table='<div class="department-table" id="table-div-'+esc(key)+'">'+title+summary+'<div class="table-responsive-wrapper"><table id="table-'+esc(key)+'"><thead><tr><th rowspan="2">م</th><th rowspan="2" class="job-title">مسمى الوظيفة</th><th rowspan="2">الفئة</th><th rowspan="2" class="employee-name">اسم شاغل الوظيفة</th><th colspan="'+p.daysInExtract+'">الأيام</th><th rowspan="2">التكلفة</th><th colspan="2">الأيام</th><th colspan="2">الحسم والغرامة</th><th rowspan="2">الصافي</th><th rowspan="2">الجنسية</th><th rowspan="2">غرامة جنسية</th><th rowspan="2">الإقامة</th></tr><tr>'+daysHeader+'<th>حضور</th><th>غياب</th><th>الحسم</th><th>غرامة الغياب</th></tr></thead><tbody>'+body+'</tbody></table></div>'+signaturesHtml('attendance',key,'signature-item')+'</div>';
    return table;
  }

  function attendancePage(key) {
    var clone = cloneOriginalAttendanceTable(key);
    var body = clone ? cleanClonedAttendance(clone, key) : fallbackAttendancePage(key);
    var contractInfo = document.querySelector('.page-contract-info-v2,.page-contract-info');
  ...
  function performanceItems(){try{if(typeof window.getDynamicPerformanceItems==='function')return window.getDynamicPerformanceItems()||[]}catch(_){}var saved=readJson('adminOfficePerformanceItems_v1',[]);return Array.isArray(saved)&&saved.length?saved:[{text:'الالتزام بتنفيذ الأعمال طبقاً للعقد',max:100}];}
  function performanceScores(key){try{if(typeof window.getPerformanceData==='function')return (window.getPerformanceData()||{})[key]||{}}catch(_){}return readJson('performanceData_v4',{})[key]||readJson('performanceData',{})[key]||{};}
  function centerCost(key){try{if(typeof window.calculateCenterTotalCost==='function')return num(window.calculateCenterTotalCost(key))}catch(_){}var p=period();return(attendanceData()[key]||[]).reduce(function(s,emp){return s+employeeCalc(emp,p).costForPeriod},0);}
  function performancePage(key){if(key==='admin_staff')return '';var officeName=officeNames()[key]||key,items=performanceItems(),scores=performanceScores(key),maxTotal=0,scoreTotal=0,rows='';items.forEach(function(item,i){var max=num(item.max||item.maxScore||item.degree||0),score=scores[i]!==undefined?num(scores[i]):max;maxTotal+=max;scoreTotal+=score;rows+='<tr><td>'+(i+1)+'</td><td class="item-text">'+esc(item.text||item.name||item.title||'')+'</td><td>'+max+'</td><td>'+score+'</td></tr>';});rows+='<tr style="font-weight:bold;background:#f0f0f0"><td colspan="2">المجموع</td><td>'+maxTotal+'</td><td>'+scoreTotal+'</td></tr>';var pct=maxTotal?(scoreTotal/maxTotal*100):100,dedPct=0;if(pct<60)dedPct=15;else if(pct<65)dedPct=10;else if(pct<70)dedPct=8;else if(pct<75)dedPct=6;else if(pct<80)dedPct=4;else if(pct<85)dedPct=2;var cost=centerCost(key),dedAmount=cost*dedPct/100;var html='<div class="performance-report">'+headerHtml(key,true)+'<div class="cert-title">جدول تقييم مستوى الأداء والإنجاز</div><div class="sub-title">لموقع: '+esc(officeName)+' - عن شهر '+esc(monthName())+'</div><div class="cost-bar">إجمالي المبلغ لأنشطة القسم: '+money(cost)+' ريال</div><table><thead><tr><th>م</th><th style="width:65%">أنشطة القسم</th><th>الدرجة القصوى</th><th>التقدير</th></tr></thead><tbody>'+rows+'</tbody></table><div class="summary"><p>التقدير الذي حصل عليه المقاول: <b>'+pct.toFixed(2)+'%</b>، نسبة الحسم: <b>'+dedPct+'%</b>، ويساوي: <b>'+money(dedAmount)+' ريال</b></p></div>'+signaturesHtml('performance',key,'sign-box')+'</div>';return '<div class="page-container portrait-page-perf">'+html+'</div>';}
  ...
  function printSelected(){var keys=selectedKeys(),opts={attendance:checked('print-opt-attendance'),performance:checked('print-opt-performance'),achievement:checked('print-opt-achievement')},letters=checked('print-opt-raise-labor')||checked('print-opt-raise-consumables')||checked('print-opt-raise-declaration');if(!keys.length||(!opts.attendance&&!opts.performance&&!opts.achievement&&!letters)){alert('الرجاء اختيار مكتب/مرفق وتقرير واحد على الأقل للطباعة.');return;}if(typeof window.closeDialog==='function')window.closeDialog('management-dialog');var w=window.open('','_blank','width=1200,height=900');if(!w)return alert('المتصفح منع فتح نافذة الطباعة.');var doc=w.document;doc.open();doc.write('<html><head><title>طباعة التقارير المجمعة</title><link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@400;500;700&display=swap" rel="stylesheet">'+printCss());if(window.AdminOfficePrintLetters&&typeof window.AdminOfficePrintLetters.lettersCss==='function')doc.write(window.AdminOfficePrintLetters.lettersCss());doc.write('</head><body>');keys.forEach(function(key){if(opts.attendance)doc.write(attendancePage(key));if(opts.performance)doc.write(performancePage(key));if(opts.achievement)doc.write(achievementPage(key));});if(letters&&window.AdminOfficePrintLetters&&typeof window.AdminOfficePrintLetters.buildSelectedLetters==='function')doc.write(window.AdminOfficePrintLetters.buildSelectedLetters());doc.write('</body></html>');doc.close();w.onload=function(){w.focus();w.print();w.close();};}
  ...
  console.info('[Admin Offices Print All] consolidated print flow v2 using original attendance table settings');
})();