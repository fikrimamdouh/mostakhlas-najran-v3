// ===================================================================
// Admin Offices Consumables Raise Letter — SAFE RESTORE V9
// Scope: admin_offices_consumables.html / original-viewer page
// تم حذف تعديل منطق زيارة مقاولي الباطن وتعديل الجدول لأنه سبب تجميد الصفحة.
// الملف الآن يضيف فقط: إعدادات خطاب الرفع + طباعة خطاب المستهلكات.
// ===================================================================
(function () {
  'use strict';
  ...
  function currentConsumablesNet() {
    const summary = readRenderedNumber('#summary-table tfoot tr:first-child td:last-child');
    ...
    return num(localStorage.getItem('admin_offices_consumables_current_net') || localStorage.getItem('adminOfficesConsumablesNet') || localStorage.getItem('finalConsumablesCost') || 0);
  }
  ...
  function saveDialog(silent) {
    ...
    if (!note) {
      ...
      document.querySelector('#consumables-raise-letter-overlay .settings-dialog')?.prepend(note);
    }
  ...
  function injectPageCss() {
    if (document.getElementById('admin-consumables-raise-letter-safe-css')) return;
    const style = document.createElement('style');
    style.id = 'admin-consumables-raise-letter-safe-css';
    style.textContent = `
  ...
  window.AdminOfficesConsumablesRaiseLetter = { openDialog, closeDialog, saveDialog, generate: generateConsumablesRaiseLetter, getCurrentConsumablesNet: currentConsumablesNet, tafqeetSAR };
  ...
  console.info('[Admin Offices Consumables Raise Letter] installed v9 safe restore — subcontractor patch removed');
})();