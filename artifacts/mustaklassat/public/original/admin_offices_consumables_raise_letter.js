...
(function () {
  ...
  function normalizeVisitDate(value) {
    const oldFutureMonths = [
      'يناير القادم', 'فبراير القادم', 'مارس القادم', 'أبريل القادم', 'مايو القادم', 'يونيو القادم',
      'يوليو القادم', 'أغسطس القادم', 'سبتمبر القادم', 'أكتوبر القادم', 'نوفمبر القادم', 'ديسمبر القادم'
    ];
  ...
  function getCurrentConsumablesNet() {
    ...
    if (summaryFooterNet > 0) {
      persistConsumablesNet(summaryFooterNet);
      return summaryFooterNet;
    }
    ...
    if (dashboardTotal > 0) {
      persistConsumablesNet(dashboardTotal);
  ...
  function saveDialog() {
    const patch = {};
    document.querySelectorAll('[data-cons-letter-setting]').forEach(el => {
      patch[el.dataset.consLetterSetting] = el.value;
    });
    saveSettings(patch);
    ...
    if (note) {
      note.textContent = 'تم حفظ إعدادات خطاب رفع المستهلكات بنجاح.';
      setTimeout(() => { if (note) note.textContent = ''; }, 2500);
    }
  }

  function openDialog() {
    if (!document.getElementById('consumables-raise-letter-overlay')) renderDialog();
  }

  function closeDialog() {
    document.getElementById('consumables-raise-letter-overlay')?.remove();
  }
  ...
  window.AdminOfficesConsumablesRaiseLetter = {
    openDialog,
    closeDialog,
    saveDialog,
    generate: generateConsumablesRaiseLetter,
    getCurrentConsumablesNet
  };
