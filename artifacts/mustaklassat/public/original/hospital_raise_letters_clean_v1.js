...
(function () {
  ...
  function writeJson(key, value) { try { localStorage.setItem(key, JSON.stringify(value || {})); return true; } catch (_) { void window.NajranDialogs.alert('تعذر الحفظ. صغّر صورة الترويسة أو صدّر الإعدادات.'); return false; } }
  ...
  function saveDoc(){var s=settings(), key=document.getElementById('hrl-doc-select')?.value||s.selectedDocKey; if (NATIVE_DOCS[key]) return void window.NajranDialogs.alert('هذا مستند تشغيلي يطبع من النظام الأصلي ولا توجد له إعدادات خطابات.'); s.selectedDocKey=key; var d=Object.assign({},s.documentSettings[key]||{}); document.querySelectorAll('#hospital-letters-clean [data-doc-field]').forEach(function(el){d[el.dataset.docField]=el.value;}); d.enabled=!!document.getElementById('hrl-doc-enabled')?.checked; d.useGlobalLetterhead=!!document.getElementById('hrl-doc-use-global-letterhead')?.checked; s.documentSettings[key]=d; saveSettings(s); UI.docOpen=true; render(); flash('تم حفظ إعداد الخطاب.');}
  function uploadHead(file){if(!file)return; if(!/^image\//.test(file.type||''))return void window.NajranDialogs.alert('ارفع صورة فقط.'); var r=new FileReader(); r.onload=function(){var s=settings(); s.letterheadDataUrl=String(r.result||''); s.letterheadEnabled=true; saveSettings(s); UI.settingsOpen=true; render();}; r.readAsDataURL(file);}
  ...
  function importSettings(file){if(!file)return; var r=new FileReader(); r.onload=function(){try{var x=JSON.parse(String(r.result||'{}')); saveSettings(Object.assign(defaults(),x,{documentSettings:x.documentSettings||{}})); render();}catch(_){void window.NajranDialogs.alert('ملف الإعدادات غير صالح.');}}; r.readAsText(file);}
  ...
  document.addEventListener('click', async function(e){var b=e.target.closest&&e.target.closest('#hospital-letters-clean [data-action]'); if(!b)return; e.preventDefault(); var a=b.dataset.action,k=b.dataset.docKey; if(k && !NATIVE_DOCS[k]){var s=settings(); s.selectedDocKey=k; saveSettings(s);} if(a==='close')return document.getElementById('hospital-letters-clean')?.remove(); if(a==='toggleSettings'){UI.settingsOpen=!UI.settingsOpen; return render();} if(a==='toggleDoc'){UI.docOpen=!UI.docOpen; return render();} if(a==='openDoc'){UI.docOpen=true; return render();} if(a==='saveGlobal')return saveGlobal(); if(a==='saveDoc')return saveDoc(); if(a==='export')return exportSettings(); if(a==='reset'){if(await window.NajranDialogs.confirm('حذف إعدادات خطابات المستشفى؟')){localStorage.removeItem(SETTINGS_KEY);render();}return;} if(DOCS.some(function(d){return d.key===a;}))return printDoc(a);},true);
