// ✅ Settings page fast-load guard
// يقلل تهنيج صفحة الإعدادات فقط بدون تغيير بيانات أو صلاحيات أو حفظ.
(function settingsMainFastLoadGuard() {
    try {
        if (!/settings_main\.html(?:$|[?#])/.test(window.location.href)) return;
        if (window.__najranSettingsFastLoadGuard) return;
        window.__najranSettingsFastLoadGuard = true;

        const nativeSetTimeout = window.setTimeout.bind(window);

        window.setTimeout = function patchedSettingsSetTimeout(callback, delay, ...args) {
            try {
                const src = typeof callback === 'function' ? Function.prototype.toString.call(callback) : '';

                // اعتراض تحميل settings_main.js الثقيل عند فتح الصفحة فقط.
                if (
                    typeof callback === 'function' &&
                    Number(delay) === 10 &&
                    src.includes('autoSelectFirstHospital') &&
                    src.includes('switchHospital') &&
                    src.includes('autoFillFromSession')
                ) {
                    return nativeSetTimeout(function lightSettingsBoot() {
                        console.log('[settings-fast] تشغيل تحميل إعدادات مخفف');
                        try {
                            if (typeof loadPersistentData === 'function') loadPersistentData();
                            if (typeof autoFillFromSession === 'function') autoFillFromSession();
                            ...
                            console.log('[settings-fast] اكتمل تحميل إعدادات الصفحة');
                        } catch(e) {
                            console.error('[settings-fast] خطأ أثناء تحميل الإعدادات:', e);
                        }
                    }, 50);
                }

                // إلغاء التكرارات الاحتياطية الثقيلة الخاصة بصفحة الإعدادات فقط.
                if (
                    typeof callback === 'function' &&
                    callback.name === 'autoFillFromSession' &&
                    (Number(delay) === 200 || Number(delay) === 600 || Number(delay) === 1200)
                ) {
                    return 0;
                }
            } catch(e) {}
