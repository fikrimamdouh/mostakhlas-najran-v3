...
// --- 3. DATA MANAGEMENT ---
function getCenterNames() { return JSON.parse(localStorage.getItem('centerNames_v3')) || {}; }
function saveCenterNames(names) { localStorage.setItem('centerNames_v3', JSON.stringify(names)); }
function getAttendanceData() { return JSON.parse(localStorage.getItem('centersAttendanceData_v2')) || {}; }
function saveAttendanceData(data) { localStorage.setItem('centersAttendanceData_v2', JSON.stringify(data)); }

// ✅✅✅ [الكود النهائي والمطابق للصورة] ✅✅✅
function initializeCenterNames() {
    const defaultNames = {
        'center_1': 'الخالدية',
        'center_2': 'الامير مشعل',
        'center_3': 'الضيافة',
        'center_4': 'الفهد الشمالى',
        'center_5': 'الفهد الجنوبى',
        'center_6': 'الضباط',
        'center_7': 'الفيصلية',
        'center_8': 'دحضة',
        'center_9': 'البلد',
        'center_10': 'ابا السعود',
        'center_11': 'الشبهان',
        'center_12': 'المراطة',
        'center_13': 'الغويلا',
        'center_14': 'العريسة',
        ...
        'center_50': 'اللجام',
        'center_51': 'وادى وسط',
        'center_52': 'خالدية شرورة',
        'center_53': 'سلطانة شرورة',
        'center_54': 'المنخلى',
        'center_55': 'تمانى',
        'center_56': 'الوديعة شرورة',
        'center_57': 'عزيزية شرورة',
        'center_58': 'المشعلية',
        'center_59': 'الخرعاء',
        'center_60': 'المنتشر',
        'center_61': 'نواقل المرض1',
        'center_62': 'الروضة الشمالية',
        'center_63': 'الكنتوب',
        'center_64': 'شعب رير',
        'center_65': 'جنوب المطار',
        'center_66': 'تصلال',
        'center_67': 'التدريب',
        'center_68': 'الاخاشيم',
        'center_69': 'نهوقة',
        'center_70': 'سكن التمريض بالمجمع',
        'center_71': 'عاكفة',
        'center_72': 'الفريق الجوال',
        'center_73': 'حبونا الجديد',
        'admin_staff': 'الوظائف الإدارية'
    };
...
function exportSelectedToExcel() {
    ...
    if (selectedKeys.length === 0) {
        void window.NajranDialogs.alert("الرجاء اختيار مركز واحد على الأقل.");
        return;
    }
...
/**
 * ✅ [النسخة النهائية V20 - مع استثناء الوظائف الإدارية من طباعة الأداء]
 * 1. (استثناء ذكي): تتجاهل الآن طباعة "شهادة تقييم الأداء" تحديداً لفئة "الوظائف الإدارية" حتى لو تم تحديدها.
...
function printSelected() {
    ...
    selectedKeys.forEach((centerKey) => {
        ...
        // =======================================================================
        // ✅✅✅ [الحل هنا] إضافة شرط للتحقق من أن المركز ليس "الوظائف الإدارية" ✅✅✅
        ...
        if (printOptions.achievement) {
            ...
            // ✅✅✅ [الحل هنا] التأكد من أن حسم الأداء لا يطبق على الوظائف الإدارية ✅✅✅
...
function getSignatureKeyForContext(type, contextKey) {
    ...
    const uniqueSignatureKeys = {
        ...
        'center_72': `${type}_special_mobile_team`,         // الفريق الجوال
        'admin_staff': `${type}_special_admin_jobs`,        // الوظائف الإدارية
...
function getSignatureDialogTitle(type, contextKey) {
    ...
    const titles = {
        ...
        'center_72': `تواقيع: ${centerNames['center_72'] || 'الفريق الجوال'}`,
        'admin_staff': `تواقيع: ${centerNames['admin_staff'] || 'الوظائف الإدارية'}`,
...
async function clearAllData() {
    ...
    if (await window.NajranDialogs.confirm("تحذير: هل أنت متأكد من رغبتك في مسح جميع بيانات الموظفين، المراكز، والتواقيع؟ لا يمكن التراجع عن هذا الإجراء.")) {
        localStorage.removeItem('centersAttendanceData_v2');
        localStorage.removeItem('centerNames_v3');
...
/**
 * ✅ [النسخة النهائية V5 - تشمل الوظائف الإدارية]
 * تحسب الإجمالي العام لجميع المراكز الصحية **بما في ذلك الوظائف الإدارية**،
...
function calculateAndDisplayGrandTotal() {
    ...
    // --- 5. [مهم للربط] حفظ صافي الاستحقاق الإجمالي (مفتاح خاص بالمراكز الصحية) ---
    localStorage.setItem('grand-net-total-centers', grandNetTotal.toFixed(2));
}
...
async function confirmAddNewCenter(andReturn = false) {
    ...
    if (!newName) {
        void window.NajranDialogs.alert("الرجاء إدخال اسم للمركز الجديد.");
        return;
    }
    if (!await window.NajranDialogs.confirm(`هل أنت متأكد من إضافة "${newName}" إلى القائمة؟`)) {
        return;
    }
    const names = getCenterNames();
...
function showGrandAchievementCertificate() {
    ...
    sortedCenterKeys.forEach(centerKey => {
        ...
        tableRows += `
            <tr>
                <td>عمالة مركز ${centerNames[centerKey]}</td>
