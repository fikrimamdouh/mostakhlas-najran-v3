(function () {
  ...
  var state = { data: null, archivePage: 1, archivePages: 1, stream: null, scanLoop: 0, scanning: false, barcodeDetector: null };
  ...
  async function freshToken() {
    ...
    for (var i = 0; i < getters.length; i++) if (typeof getters[i] === 'function') { try { var value = await getters[i]({ skipCache: true }); if (value) return value; } catch (_) {} }
    try { var session = JSON.parse(localStorage.getItem('najran_session') || '{}'); if (session.clerkToken && Date.now() - Number(session.timestamp || 0) < 55000) return session.clerkToken; } catch (_) {}
  ...
  async function loadBootstrap() {
    try {
      state.data = await api('/management/bootstrap');
      renderAll();
    } catch (error) {
  ...
  function populateForms() {
    var d = state.data;
    var systems = d.systems.filter(function (x) { return x.isActive; }), contractors = d.contractors.filter(function (x) { return x.isActive; });
    ...
    var sites = Array.from(new Set(d.siteApprovals.map(function (x) { return x.siteName; }))).filter(Boolean);
    document.querySelectorAll('select[name="siteName"],select[name="sourceSite"]').forEach(function (select) { var current = select.value; select.innerHTML = '<option value="">اختر الموقع</option>' + sites.map(function (site) { return '<option' + (site === current ? ' selected' : '') + '>' + esc(site) + '</option>'; }).join(''); });
  ...
  document.addEventListener('click', async function (event) {
    var target = event.target.closest('[data-toggle-system],[data-toggle-contractor],[data-toggle-approval],[data-toggle-qualification],[data-toggle-rep],[data-rep-systems],[data-upload-doc],[data-download-doc],[data-disable-doc],[data-link],[data-reject],[data-preview],[data-visit-detail]'); if (!target) return;
    try {
  ...
  function linkFormHtml(visit) {
    var d = state.data, start = visit.startsAt ? new Date(visit.startsAt).toISOString().slice(0, 16) : new Date(String(visit.visitDate) + 'T08:00:00').toISOString().slice(0, 16), end = visit.endsAt ? new Date(visit.endsAt).toISOString().slice(0, 16) : new Date(String(visit.visitDate) + 'T17:00:00').toISOString().slice(0, 16);
    return '<form id="link-form" class="form-grid"><div class="field"><label>النظام</label><select name="systemId" required>' + optionRows(d.systems.filter(function(x){return x.isActive;}),function(x){return x.name;}) + '</select></div><div class="field"><label>الشركة</label><select name="contractorId" required>' + optionRows(d.contractors.filter(function(x){return x.isActive;}),function(x){return x.name;}) + '</select></div><div class="field"><label>المندوب</label><select name="representativeId" required>' + optionRows(d.representatives.filter(function(x){return x.isActive;}),function(x){return x.fullName+' — '+x.identityMasked;}) + '</select></div><div class="field"><label>اعتماد الموقع</label><select name="siteApprovalId" required>' + optionRows(d.siteApprovals.filter(function(x){return x.status==='active';}),function(x){return x.siteName+' #'+x.id;}) + '</select></div><div class="field"><label>التأهيل</label><select name="qualificationId" required>' + optionRows(d.qualifications.filter(function(x){return x.status==='active';}),function(x){return 'تأهيل #'+x.id;}) + '</select></div><div class="field"><label>البداية</label><input type="datetime-local" name="startsAt" value="' + esc(start) + '" required></div><div class="field"><label>النهاية</label><input type="datetime-local" name="endsAt" value="' + esc(end) + '" required></div><div class="field full"><label>الغرض من الزيارة</label><textarea name="purpose" required>' + esc(visit.purpose || '') + '</textarea></div></form>';
  }
  function openLink(id) { var visit = state.data.pending.find(function (x) { return x.id === id; }); if (!visit) return; modal('ربط الطلب واعتماده', linkFormHtml(visit), [{ label: 'حفظ الربط واعتماد التصريح', className: 'btn-green', action: async function (button) { button.disabled = true; try { var body = formObject(document.getElementById('link-form')); body.startsAt = new Date(body.startsAt).toISOString(); body.endsAt = new Date(body.endsAt).toISOString(); await api('/' + id + '/link', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); await api('/' + id + '/status', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'approved' }) }); closeModal(); toast('تم الربط والاعتماد وإصدار رقم جديد', true); await loadBootstrap(); } catch (e) { toast(e.message, false); button.disabled = false; } } }]); }
  ...
  document.querySelector('#direct-form [name="contractorId"]').addEventListener('change', refreshDirectRepresentatives); document.querySelector('#direct-form [name="systemId"]').addEventListener('change', refreshDirectRepresentatives);
  document.getElementById('direct-form').addEventListener('submit', async function (event) {
    event.preventDefault(); var body = formObject(this); body.systemId = Number(body.systemId); body.contractorId = Number(body.contractorId); body.representativeId = Number(body.representativeId); body.startsAt = new Date(body.startsAt).toISOString(); body.endsAt = new Date(body.endsAt).toISOString();
  ...
  async function openVisitDetail(id) { try { var body = await api('/' + id), v = body.visit; var details = [['رقم التصريح',v.serialNumber],['الحالة',statusLabel(v.effectiveStatus)],['الزائر',v.repName],['الهوية / الإقامة',v.repIdMasked],['الشركة',v.subContractor],['النظام',v.systemName],['الموقع',v.siteLocation],['الغرض',v.purpose],['البداية',dateText(v.startsAt,true)],['النهاية',dateText(v.endsAt,true)],['سبب الإلغاء',v.cancelledReason]], documents = body.documents || []; var docHtml = '<div class="card" style="margin-top:12px"><h3>الوثائق</h3>' + (documents.length ? documents.map(function(doc){var disable=doc.status==='active'?'<button class="btn btn-red" data-disable-doc="'+doc.id+'">تعطيل</button>':'';return '<div class="alert-row" style="background:#f8fafc;border-color:#e2e8f0;color:#334155"><span>'+esc(doc.documentType)+' — '+esc(doc.originalName)+' '+badge(doc.status==='active'?'active':'cancelled')+'</span><span class="actions"><button class="btn btn-light" data-download-doc="'+doc.id+'" data-name="'+esc(doc.originalName)+'">تنزيل محمي</button>'+disable+'</span></div>';}).join('') : '<div class="empty">لا توجد وثائق مرتبطة بالزيارة</div>') + '</div>'; modal('ملف الزيارة', '<div class="detail-grid">' + details.filter(function(x){return x[1];}).map(function (row) { return '<div class="detail"><small>' + esc(row[0]) + '</small><strong>' + esc(row[1]) + '</strong></div>'; }).join('') + '</div>' + docHtml, [{ label:'رفع وثيقة',className:'btn-light',action:function(){openDocumentUpload('visit',id);} },{ label:'معاينة التصريح',className:'btn-primary',action:function(){window.NajranVisitPermit.preview(id).catch(window.NajranVisitPermit.showError);} },{ label:'إعادة إصدار',className:'btn-green',action:function(){openReissue(v);} },{ label:'إلغاء الزيارة',className:'btn-red',action:function(){openCancel(v);} }]); } catch (e) { toast(e.message,false); } }
  function openReissue(v) { var start = new Date(Date.now()+86400000); start.setHours(8,0,0,0); var end = new Date(start); end.setHours(17,0,0,0); modal('إعادة إصدار زيارة برقم جديد','<form id="reissue-form" class="form-grid"><div class="field"><label>البداية</label><input type="datetime-local" name="startsAt" value="'+start.toISOString().slice(0,16)+'" required></div><div class="field"><label>النهاية</label><input type="datetime-local" name="endsAt" value="'+end.toISOString().slice(0,16)+'" required></div><div class="field full"><label>الغرض</label><textarea name="purpose" required>'+esc(v.purpose||'')+'</textarea></div></form>',[{label:'إنشاء تصريح جديد',className:'btn-green',action:async function(){var body=formObject(document.getElementById('reissue-form'));body.startsAt=new Date(body.startsAt).toISOString();body.endsAt=new Date(body.endsAt).toISOString();try{var result=await api('/'+v.id+'/reissue',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});closeModal();toast('تم إنشاء تصريح جديد مع الحفاظ على الأصل',true);loadArchive(state.archivePage);window.NajranVisitPermit.preview(result.visit.id).catch(window.NajranVisitPermit.showError);}catch(e){toast(e.message,false);}}}]); }
  ...
  function fileData(file) { return new Promise(function (resolve, reject) { if (!file) return resolve(undefined); var reader = new FileReader(); reader.onload = function () { resolve(reader.result); }; reader.onerror = reject; reader.readAsDataURL(file); }); }
  document.getElementById('print-form').addEventListener('submit', async function (event) { event.preventDefault(); try { var body = { managerName: this.elements.managerName.value.trim() }, stamp = await fileData(this.elements.stampFile.files[0]), signature = await fileData(this.elements.signatureFile.files[0]); if (stamp !== undefined) body.stamp = stamp; if (signature !== undefined) body.signature = signature; await api('/settings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); toast('تم حفظ إعدادات الطباعة', true); } catch (e) { toast(e.message, false); } });
  ...
  function tokenFromValue(value) { try { var url = new URL(value, location.origin); return url.searchParams.get('token') || (url.pathname === '/api/visits/qr/verify-link' ? '' : value); } catch (_) { return value; } }
