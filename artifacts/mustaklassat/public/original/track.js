...
(function () {
  ...
  window.saveEditedPerformanceItems = function (centerKey) {
    ...
    if (!newItems.length) return void window.NajranDialogs.alert('لا يمكن حفظ قائمة بنود فارغة.');
  ...
  window.resetPerformanceScores = async function (centerKey) {
    if (!await window.NajranDialogs.confirm('هل تريد إعادة درجات تقييم الأداء لهذا المكتب/المرفق إلى الدرجة الكاملة؟')) return;
  ...
  window.printSelected = function () { var selected = Array.from(document.querySelectorAll('#print-centers-checkboxes input:checked')).map(function (x) { return x.value; }); if (!selected.length) return void window.NajranDialogs.alert('الرجاء اختيار مكتب/مرفق واحد على الأقل للطباعة.'); var pages = selected.map(function (key) { return buildAttendancePage(key); }); var win = window.open('', '', 'width=1200,height=900'); var doc = win.document; doc.open(); doc.write('<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><title>طباعة حضور المكاتب</title><link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@400;500;700&display=swap" rel="stylesheet">' + buildPrintCss() + '</head><body>' + pages.join('') + '</body></html>'); doc.close(); setTimeout(function () { win.focus(); win.print(); }, 500); };
  ...
  window.printSelectedAdminOfficeReports = function () { var selected = Array.from(document.querySelectorAll('.admin-office-print-center:checked')).map(function (cb) { return cb.value; }); if (!selected.length) return void window.NajranDialogs.alert('الرجاء اختيار مكتب/مرفق واحد على الأقل.'); var host = document.createElement('div'); host.id = 'print-centers-checkboxes'; host.style.display = 'none'; host.innerHTML = selected.map(function (key) { return '<input type="checkbox" value="' + esc(key) + '" checked>'; }).join(''); document.body.appendChild(host); closeAdminPrintDialog(); try { window.printSelected(); } finally { setTimeout(function () { host.remove(); }, 1000); } };
