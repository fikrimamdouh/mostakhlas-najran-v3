    ...
    <div style={{ borderBottom: "1px solid #f1f5f9", padding: "11px 16px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
          <button onClick={() => { setMultiOpen(v => !v); setOpen(false); }} title="تعيين مواقع متعددة"
          ...
          <button onClick={() => { setOpen(v => !v); setMultiOpen(false); }} title="نقل إلى موقع آخر"
    ...
    <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 2px 16px rgba(30,60,114,.08)", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <div style={{ background: "linear-gradient(135deg,#1e3c72,#2a5298)", padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
          ...
          <span style={{ background: "rgba(255,255,255,0.18)", color: "#fff", borderRadius: 99, padding: "2px 10px", fontSize: "0.76rem", fontWeight: 700 }}>{users.length} إجمالي</span>
        </div>
      </div>
      <div style={{ flex: 1 }}>
        {users.length === 0 ? (
        ...
        ) : users.map(u => <UserRow key={u.id} user={u} onReassign={onReassign} onMultiAssign={onMultiAssign} mutatingId={mutatingId} />)}
      </div>
    </div>
  );
}
    ...
    <div style={{ direction: "rtl", fontFamily: "'Segoe UI', Arial, sans-serif", minHeight: "100vh", background: "#f0f4f8", padding: "24px 20px" }}>
      <div style={{ background: "linear-gradient(135deg,#0f2050,#1e3c72,#2a5298)", borderRadius: 16, padding: "22px 28px", marginBottom: 22, color: "#fff" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ margin: "4px 0 0", opacity: 0.7, fontSize: "0.82rem" }}>اعرض وعدّل ربط كل موظف بموقعه أو الإدارة — التغييرات تسري فوراً</p>
          </div>
      ...
      {filterUsers(unassigned).length > 0 && (
        <div style={{ background: "#fffbeb", border: "1.5px solid #fcd34d", borderRadius: 14, padding: "16px 20px", marginBottom: 22 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14, color: "#92400e", fontWeight: 800, fontSize: "0.95rem" }}>
            <AlertCircle size={18} /> {filterUsers(unassigned).length} مستخدم بدون ربط
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 12 }}>
            {filterUsers(unassigned).map(u => <div key={u.id} style={{ background: "#fff", borderRadius: 10, border: "1.5px solid #fde68a", overflow: "hidden" }}><UserRow user={u} onReassign={handleReassign} onMultiAssign={handleMultiAssign} mutatingId={mutatingId} /></div>)}
          </div>
        </div>
      )}
        ...
        {COMPANY_TABS.map(({ key, label, icon }) => {
          ...
          return (
            <button key={key} onClick={() => setActiveCompany(key)}
              ...
              {icon} {label}
              <span style={{ marginRight: 8, opacity: 0.8, fontSize: "0.8rem" }}>({count} موظف / {COMPANY_SITES[key].length} موقع)</span>
            </button>
          );
        })}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: 18 }}>
        {currentSites.map(hospitalName => (
          <HospitalCard key={hospitalName} name={hospitalName} users={filterUsers(byHospital[hospitalName] || [])} onReassign={handleReassign} onMultiAssign={handleMultiAssign} mutatingId={mutatingId} />
        ))}
      </div>
    </div>
  );
