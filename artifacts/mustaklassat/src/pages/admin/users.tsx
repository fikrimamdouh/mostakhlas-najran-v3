...
// ── نافذة تأكيد مسح المستخلصات مع الحفاظ على الزيارات ───────────────────────
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.55)", direction: "rtl" }}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="p-5 flex items-center gap-3" style={{ background: "linear-gradient(135deg,#92400e,#d97706)", color: "#fff" }}>
          ...
          <div>
            <h2 className="text-lg font-bold">مسح المستخلصات</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              <Trash2 className="h-4 w-4" />
              {isPending ? "جاري المسح..." : "مسح المستخلصات فقط"}
            </Button>
          </div>
        </div>
      </div>
...
export default function AdminUsers() {
  ...
  const resetExtracts = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "✅ تم المسح", description: "تم حذف جميع المستخلصات والمشاريع مع الحفاظ على طلبات الزيارة وسجلاتها." });
