...
import { COMMON_PAGE_KEYS, PAGE_FILTERS, SETTINGS_STORAGE_KEYS, SETTINGS_STORAGE_SET, uniqueStorageList } from "../lib/storage-scope";
...
function requestedFilters(req: any): { keys: string[]; prefixes: string[]; scope: string } | null {
  ...
  if (pageFilter) {
    return {
      keys: uniqueStorageList(COMMON_PAGE_KEYS.concat(pageFilter.keys || [])),
      prefixes: uniqueStorageList(pageFilter.prefixes || []),
      scope: `page:${page}`
    };
  }
