...
export const COMPANY_SITES: Record<string, { sites: string[] }> = {
  ...
  "بيت_العرب": { sites: ["مستشفى يدمه العام", "مستشفى يدمة العام", "مستشفى حبونا العام", "مستشفى بدر الجنوب العام", "مستشفى الولادة والأطفال", "مستشفى غرب نجران للولادة والأطفال والعيادات التخصصية", "المكاتب الإدارية والمرافق الصحية وصيانة وإصلاح السيارات والعيادات المتنقلة", "مستشفى نجران العام القديم وسكن الممرضات الخارجي", "المكاتب الإدارية والمرافق الصحية", "صيانة وإصلاح السيارات والعيادات المتنقلة"] },
  "سراكو": { sites: ["مستشفى نجران العام الجديد ومركز طب الأسنان التخصصي", "مستشفى نجران العام الجديد", "مركز طب الأسنان التخصصي", "مجمع الأمل للصحة النفسية", "مستشفى ثار العام", "مستشفى خباش العام", "المراكز الصحية", "مستشفى الملك خالد", "مركز الأمير سلطان", "مستشفى شروره العام"] },
  "تجمع_نجران": { sites: ["المقر الرئيسي — تجمع نجران الصحي"] },
};

export function companySitesForKey(companyKey: unknown): string[] {
  const key = String(companyKey || "").trim();
  return key ? [...(COMPANY_SITES[key]?.sites ?? [])] : [];
}

export function companySitesFor(user: any): string[] {
  return companySitesForKey(user?.contractCompany);
}
