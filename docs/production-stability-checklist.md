...
## اختبارات قبول P0 — تثبيت الإنتاج (idempotency / versioning / نطاق الصلاحيات)

> تُنفَّذ على staging قبل أي deploy، وبعد تشغيل `scripts/migrate-production.sql`.

1. **رفع نفس المستخلص من جهازين في نفس الوقت**
   جهازان بنفس المستخدم، نفس النوع/الشهر/السنة/رقم الدفعة، ضغط «رفع» متزامن.
   ✅ النتيجة: سجل واحد فقط في `submitted_extracts`؛ الجهاز الثاني يستقبل `409` برسالة
   «تم رفع نفس المستخلص مسبقًا» مع `existingId`. لا سجل مكرر مهما تكرر الضغط.

2. **تعديل نفس hospital_storage من جهازين**
   الجهاز A يسحب الصفحة، الجهاز B يسحب ثم يحفظ تعديلًا، ثم يحفظ A.
   ✅ النتيجة: A يستقبل `409 Conflict` مع قائمة المفاتيح المتعارضة؛ بيانات B **لا** تُستبدل؛
   localStorage في A لا يُمسح؛ رسالة «حدّث الصفحة لسحب أحدث نسخة» تظهر عند الرفع الصريح.

3. **فتح تعديل مستخلص قديم مع localStorage ممتلئ**
   املأ تخزين المتصفح حتى الحد ثم افتح «تعديل» من صفحة المتابعة.
   ✅ النتيجة: لا تحويل لصفحة التعديل؛ رسالة فشل كتابة واضحة تذكر عدد المفاتيح الفاشلة؛
   مفاتيح `najran_revision_*` تُنظَّف بالكامل (لا وضع تعديل جزئي عالق).

4. **contract_supervisor يرى ويفتح فقط مستخلصات شركته**
   ✅ القائمة (`GET /api/submitted-extracts` و`-lite`) ترجع مواقع شركته فقط.
   ✅ `GET /:id` و`GET /:id/revisions` لمستخلص خارج شركته → `403`.
   ✅ `PATCH /:id/status` من contract_supervisor → `403` دائمًا.
   ✅ supervisor له `supervised_hospital` محدد يحاول تغيير حالة مستخلص مستشفى آخر → `403`.

5. **قائمة 1000 مستخلص لا تسحب extractData**
   ✅ استعلام SQL للقائمتين لا يتضمن عمود `extract_data` (تحقق من `EXPLAIN`/logs).
   ✅ حجم استجابة القائمة لـ1000 صف أقل من ~1MB؛ `adminOfficePart` يأتي من العمود الصريح.

6. **مستخلص admin_offices كبير لا يتجاوز حد payload**
   أرسل `extractData` أكبر من 4MB أو يحتوي `data:image...;base64`.
   ✅ النتيجة: `413` أو `400` برسالة عربية واضحة؛ لا صف جديد في القاعدة؛
   لا تُخزَّن أي صورة base64 داخل `submitted_extracts`.

7. **Review Only لا يكتب أي PUT**
   جلسة مراجعة (`hospital=` مختلف ضمن reviewHospitals).
   ✅ لا يصدر أي `PUT /api/hospital-storage` من العميل؛ ومحاولة PUT يدوية مع
   `?hospital=` → `403 Cannot write to requested review hospital`.

8. **hospital switch لا يرفع بيانات المستشفى القديم**
   بدّل المستشفى ثم راقب الشبكة.
   ✅ تنظيف السياق يسبق أي push؛ لا PUT يحمل مفاتيح تشغيلية للمستشفى السابق؛
   المفاتيح المسيّجة بـ`__extractCtx::` لا تختلط بين السياقات (تصفية DB).

9. **snapshot نهائي للعمالة يعرض نفس الرقم بدون إعادة حساب**
   افتح تعديل مستخلص عمالة معتمد الرفع سابقًا.
   ✅ `najran_revision_previous_total_amount` يطابق `totalAmount` المخزن؛
   فحص integrity يمر؛ الإجمالي المعروض في صفحة التعديل يطابق المخزن دون احتساب جديد.

10. **restore backup على staging**
    استرجع أحدث `scheduled_backups` على staging ثم شغّل الهجرة.
    ✅ الهجرة تمر بدون أخطاء (idempotent)؛ unique index يُبنى بنجاح (جزئي على غير-NULL)؛
    backfill الأعمدة الصريحة يتجاوز JSON التالف بـNOTICE ولا يُفشل السكربت؛
    السجلات القديمة تظهر في القوائم بشكل سليم مع `adminOfficePart` مملوء حيث أمكن.
